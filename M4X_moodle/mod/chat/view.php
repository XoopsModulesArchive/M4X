...
// $Id: view.php,v 1.25 2004/08/22 14:38:39 gustav_delius Exp $
...
print_header_simple(
    ...
    '',
    true,
    update_module_button($cm->id, $course->id, $strchat),
    navmenu($course, $cm)
);

if (($chat->studentlogs or isteacher($course->id)) and !isguest()) {
    echo "<p align=right><a href=\"report.php?id=$cm->id\">" . get_string('viewreport', 'chat') . '</a></p>';
}
...
if (!isguest()) {
    ...
    link_to_popup_window(
        "/mod/chat/gui_$CFG->chat_method/index.php?id=$chat->id$groupparam",
        "chat$course->id$chat->id$groupparam",
        (string)$strenterchat,
        500,
        700,
        $strchat
