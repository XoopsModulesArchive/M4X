...
// $Id: post.php,v 1.51.2.2 2004/11/01 12:47:38 moodler Exp $
...
if ($post = data_submitted()) {
    ...
    $post->attachment = $_FILES['attachment'] ?? null;
    ...
    if (!$post->subject or !$post->message) {
    ...
    } elseif ($post->edit) {           // Updating a post
        $post->id = $post->edit;
        ...
        if (forum_update_post($post)) {
            add_to_log(
                ...
                'forum',
                'update post',
                "discuss.php?d=$post->discussion&parent=$post->id",
                (string)$post->id,
                $cm->id
            );
        ...
        exit;
    } elseif ($post->discussion) { // Adding a new post to an existing discussion
...
if (isset($post->error)) {     // User is re-editing a failed posting
...
} elseif (isset($forum)) {      // User is starting a new discussion in a forum
    ...
    if (isteacheredit($course->id) and 0 == $post->groupid) {
        $post->groupid = -1;
    }

    forum_set_return();
} elseif (isset($reply)) {      // User is writing a new reply
    ...
    if (!(mb_substr($post->subject, 0, mb_strlen($strre)) == $strre)) {
        $post->subject = $strre . ' ' . $post->subject;
    }

    unset($SESSION->fromdiscussion);
} elseif (isset($edit)) {  // User is editing their own post
    ...
    unset($SESSION->fromdiscussion);
} elseif (isset($delete)) {  // User is deleting a post
    ...
    if (isset($confirm)) {    // User has confirmed the delete
        if ($post->totalscore) {
            notice(
                get_string('couldnotdeleteratings', 'forum'),
                forum_go_back_to("discuss.php?d=$post->discussion")
            );
        } elseif (record_exists('forum_posts', 'parent', $delete)) {
            error(
            ...
            if (!$post->parent) {  // post is a discussion topic as well, so delete discussion
                ...
                redirect(
                    "view.php?f=$discussion->forum",
                    get_string('deleteddiscussion', 'forum'),
                    1
                );
            } elseif (forum_delete_post($post)) {
                add_to_log(
                    $discussion->course,
                    'forum',
                    'delete post',
                    "discuss.php?d=$post->discussion",
                    (string)$post->id,
                    $cm->id
                );
    ...
    die;
} elseif (isset($prune)) {  // Teacher is pruning
    ...
    if (isset($_REQUEST['name'])) {    // User has confirmed the prune
        ...
        $lastpost = get_record_sql(
            "SELECT MAX(modified) AS time
        ...
        print_header_simple(
            ...
            "<a href=\"../forum/index.php?id=$course->id\">$strforums</a> -> 
                          ...
                          <a href=\"discuss.php?d=$discussion->id\">$post->subject</a> -> " . get_string('prune', 'forum'),
            '',
            '',
            true,
            '',
...
if ($post->parent) {
    ...
    $navtail = (string)$toppost->subject;
}
...
if ($course->category) {
    print_header(
        "$course->shortname: $discussion->name: $toppost->subject",
