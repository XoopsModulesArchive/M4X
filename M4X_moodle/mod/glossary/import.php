...
// $Id: import.php,v 1.11 2004/08/21 20:20:56 gustav_delius Exp $
...
if ($xml = glossary_read_imported_file($file['tmp_name'])) {
    ...
    for ($i = 0, $iMax = count($xmlentries); $i < $iMax; $i++) {
        ...
        if ($newentry->concept and $newentry->definition) {
            if (!$glossary->allowduplicatedentries) {
                ...
                if ($newentry->casesensitive) {
                    ...
                    $dupentry = get_record('glossary_entries', 'ucase(concept)', mb_strtoupper($newentry->concept), 'glossaryid', $glossary->id);
                }
        ...
        if ($permissiongranted) {
            ...
            if ($newentry->id = insert_record('glossary_entries', $newentry)) {
                ...
                for ($k = 0, $kMax = count($xmlaliases); $k < $kMax; $k++) {
                    /// Importing aliases
                ...
                if ($catsincl) {
                    ...
                    for ($k = 0, $kMax = count($xmlcats); $k < $kMax; $k++) {
                        $xmlcat = $xmlcats[$k];
                        ...
                        if (!$category = get_record('glossary_categories', 'glossaryid', $glossary->id, 'name', $newcat->name)) {
                            ...
                            if (!$category->id = insert_record('glossary_categories', $category)) {
                                ...
                                $rejections .= '<tr><td>&nbsp;<strong>' . get_string('category', 'glossary') . ":</strong>$newcat->name</td>" . '<td>' . get_string('cantinsertcat', 'glossary') . '</td></tr>';
                            } else {
                                $importedcats++;
                            }
                        }
                        ...
                        if ($category) {
                            ...
                            if (!insert_record('glossary_entries_categories', $entrycat)) {
                                ...
                                $rejections .= '<tr><td>&nbsp;<strong>' . get_string('category', 'glossary') . ":</strong>$newcat->name</td>" . '<td>' . get_string('cantinsertrel', 'glossary') . '</td></tr>';
                            }
                        }
                    }
                }
            ...
            if ($newentry->concept and $newentry->definition) {
                ...
                $rejections .= "<tr><td>$newentry->concept</td>" . '<td>' . get_string('duplicateentry', 'glossary') . '</td></tr>';
            } else {
                // add to exception report (no concept or definition found))

                $rejections .= '<tr><td>---</td>' . '<td>' . get_string('noconceptfound', 'glossary') . '</td></tr>';
            }
        }
    }
