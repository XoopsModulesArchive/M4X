...
// $Id: exportentry.php,v 1.7.2.1 2004/08/29 22:46:31 stronk7 Exp $
...
if ($PermissionGranted) {
    ...
    if (!$confirm) {
        ...
        notice_yesno(
            "<center><h2>$entry->concept</h2><p align=center>$areyousure<br><b>$mainglossary->name</b>?",
            "exportentry.php?id=$id&mode=$mode&hook=$hook&entry=$entry->id&confirm=1",
            "view.php?id=$cm->id&mode=$mode&hook=$hook"
        );
    } else {
        if (!$mainglossary->allowduplicatedentries) {
            $dupentry = get_record('glossary_entries', 'glossaryid', $mainglossary->id, 'UCASE(concept)', mb_strtoupper($entry->concept));
