...
<form name="amendtitleform" action="submissions.php" method="post">
    ...
    echo "</center></form>\n";
            ...
            /******************* admin confirm delete ************************************/ elseif ('adminconfirmdelete' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
                ...
                notice_yesno(
                    get_string('confirmdeletionofthisitem', 'workshop', get_string('submission', 'workshop')),
                    "submissions.php?action=admindelete&id=$cm->id&sid=$_GET[sid]",
                    "submissions.php?id=$cm->id&action=adminlist"
                );
            } /******************* admin delete ************************************/ elseif ('admindelete' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
            ...
            } /******************* list all submissions ************************************/ elseif ('adminlist' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
            ...
            } /******************* admin update title ************************************/ elseif ('adminupdatetitle' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
                ...
                print_continue("submissions.php?id=$cm->id&action=adminlist");
            } /*************** analysis of assessments (by teacher) ***************************/ elseif ('analysisofassessments' == $action) {
                ...
                echo "<TR><td COLSPAN=\"2\" bgcolor=\"$THEME->cellheading2\"><CENTER><B>" . get_string('optionsusedinanalysis', 'workshop') . "</B></CENTER></TD></TR>\n";

                echo '<tr><td align="right">' . get_string(
                    'loadingforteacherassessments',
                    'workshop',
                    $course->teacher
                ) . ":</td>\n";
                ...
                $ntodrop = (int)(($assessmentstodrop * $nassessments / 100.0) + 0.5);
                    ...
                    if (!$loop) {
                        ...
                        foreach ($assessors as $userid => $error) {
                            ...
                            foreach ($assessments as $assessment) {
                                ...
                                for ($i = 0; $i < $workshop->nelements; $i++) {
                                    $grade = random_int(0, $maxscore[$i]);

                                    if ($sd[$i] > $minvar) {
                                        $temp = ($mean[$submission->id][$i] - $grade) * $WORKSHOP_EWEIGHTS[$weight[$i]] / $sd[$i];
                                    } else {
                                        $temp = 0;
                                    }
                ...
                execute_sql(
                    "UPDATE {$CFG->prefix}workshop_assessments SET donotuse = 0 
                ...
                $table->head = [
                    ' ',
                    get_string('name'),
                    get_string('averageerror', 'workshop'),
                    get_string('averageerror', 'workshop') . '<br>' . get_string('excludingdroppedassessments', 'workshop'),
                ...
                foreach ($assessors as $userid => $error) {
                    ...
                    if (empty($ndropped[$userid])) {
                        $numtext = (string)$nsubmissions[$userid];
                    } else {
                ...
                $table->head = [
                    ...
                    get_string('assessmentsdone', 'workshop'),
                ...
                $table->data[] = [
                    '<b>' . get_string('weight', 'workshop') . '</b>',
                    ' ',
                    ' ',
                    '<b>' . $WORKSHOP_FWEIGHTS[$workshop->gradingweight] . '</b>',
                    ' ',
                    ' ',
                    '<b>1</b>',
                    ' ',
                ...
                foreach ($students as $user) {
                    ...
                    if ($submissions = workshop_get_user_submissions($workshop, $user)) {
                        foreach ($submissions as $submission) {
                            ...
                            $finalgrade = ($assessmentgrade * $WORKSHOP_FWEIGHTS[$workshop->gradingweight] + $submissiongrade) / ($WORKSHOP_FWEIGHTS[$workshop->gradingweight] + 1.0);
                        ...
                        $finalgrade = ($assessmentgrade * $WORKSHOP_FWEIGHTS[$workshop->gradingweight]) / ($WORKSHOP_FWEIGHTS[$workshop->gradingweight] + 1.0);
            ...
            } /*************** calculate final grades (by teacher) ***************************/ elseif ('calculatefinalgrades' == $action) {
                $form = (object)$_POST;
                ...
                if (isset($form->teacherweight)) {
                    ...
                    set_field('workshop', 'teacherweight', $teacherweight, 'id', (string)$workshop->id);
                }
                ...
                if (isset($form->peerweight)) {
                    ...
                    set_field('workshop', 'peerweight', $peerweight, 'id', (string)$workshop->id);
                }
                ...
                if (isset($form->includeteachersgrade)) {
                    ...
                    set_field('workshop', 'includeteachersgrade', $includeteachersgrade, 'id', (string)$workshop->id);
                }
                ...
                if (isset($form->biasweight)) {
                    ...
                    set_field('workshop', 'biasweight', $biasweight, 'id', (string)$workshop->id);
                }
                ...
                if (isset($form->reliabilityweight)) {
                    ...
                    set_field('workshop', 'reliabilityweight', $reliabilityweight, 'id', (string)$workshop->id);
                }
                ...
                if (isset($form->gradingweight)) {
                    ...
                    set_field('workshop', 'gradingweight', $gradingweight, 'id', (string)$workshop->id);
                }

                // save number of entries in showleaguetable option

                if ('All' == $form->nentries) {
                    $form->nentries = 99;
                }

                set_field('workshop', 'showleaguetable', $form->nentries, 'id', (string)$workshop->id);

                // save the anonymous option

                set_field('workshop', 'anonymous', $form->anonymous, 'id', (string)$workshop->id);
                ...
                if ((($workshop->ntassessments >= 3) or ($workshop->nsassessments >= 3)) and ($useteachersgrades or $usepeergrades) and $biasweight) {
                    $usebiasgrades = 1;
                } else {
                    $usebiasgrades = 0;
                }
                ...
                if ((($workshop->ntassessments >= 3) or ($workshop->nsassessments >= 3)) and ($useteachersgrades or $usepeergrades) and $reliabilityweight) {
                    ...
                    foreach ($users as $user) {
                        if ($submissions = workshop_get_user_submissions($workshop, $user)) {
                            foreach ($submissions as $submission) {
                                ...
                                if ($nteachergrades) {
                                    $teachergrade = (int)($sumteachergrades / $nteachergrades + 0.5);
                                ...
                                if ($npeergrades) {
                                    $peergrade = (int)($sumpeergrades / $npeergrades + 0.5);
                                ...
                                if ($teacherweight + $peerweight > 0) {
                                    $grade = (int)((($useteachersgrades * $teachergrade * $WORKSHOP_FWEIGHTS[$teacherweight] + $usepeergrades * $peergrade * $WORKSHOP_FWEIGHTS[$peerweight]) / ($useteachersgrades * $WORKSHOP_FWEIGHTS[$teacherweight] + $usepeergrades
                                                                                                                                                                                                                                                           * $WORKSHOP_FWEIGHTS[$peerweight])) + 0.5);
                                } else {
                                    $grade = 0;
                                }
            ...
            <TD BGCOLOR=\"$THEME->cellheading2\"><B>$course->student</B></TD>\n";
                ...
                foreach ($users as $user) {
                    ...
                    if ($submissions = workshop_get_user_submissions($workshop, $user)) {
                        foreach ($submissions as $submission) {
                            $grade = ($submission->teachergrade * $WORKSHOP_FWEIGHTS[$teacherweight] + $submission->peergrade * $WORKSHOP_FWEIGHTS[$peerweight]) / ($WORKSHOP_FWEIGHTS[$teacherweight] + $WORKSHOP_FWEIGHTS[$peerweight]);
                    ...
                    if ($maxbias) {
                        $biasgrade = max(
                            ($nbias[$user->id] / $workshop->nsassessments) - (abs($bias[$user->id]) / $maxbias),
                            0.0
                        );
                    } else {
                        $biasgrade = 0;
                    ...
                    if ($dumbreliability and $workshop->nsassessments) {
                        ...
                        $reliabilitygrade = max(
                            ($nbias[$user->id] / $workshop->nsassessments) - ($reliability[$user->id] / $dumbreliability),
                            0.0
                        );
                    } else {
                        $reliabilitygrade = 0;
                    ...
                    $biasscaled = (int)($biasgrade * $workshop->grade + 0.5);

                    $reliabilityscaled = (int)($reliabilitygrade * $workshop->grade + 0.5);

                    $gradingscaled = (int)($gradinggrade[$user->id] * $workshop->grade / COMMENTSCALE + 0.5);

                    $finalgrade = (int)((($teachergrade * $WORKSHOP_FWEIGHTS[$teacherweight] * $useteachersgrades)
                                         + ($peergrade * $WORKSHOP_FWEIGHTS[$peerweight] * $usepeergrades)
                                         + ($biasscaled * $WORKSHOP_FWEIGHTS[$biasweight] * $usebiasgrades)
                                         + ($reliabilityscaled
                                            * $WORKSHOP_FWEIGHTS[$reliabilityweight]
                                            * $usereliabilitygrades)
                                         + ($gradingscaled
                                            * $WORKSHOP_FWEIGHTS[$gradingweight]
                                            * $usegradinggrades)
                                         + 0.5) / (($WORKSHOP_FWEIGHTS[$teacherweight] * $useteachersgrades)
                ...
                print_continue("view.php?a=$workshop->id");
            } /*************** display final grades (by teacher) ***************************/ elseif ('displayfinalgrades' == $action) {
            ...
            elseif ('displayfinalweights' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }

                if (5 != $workshop->phase) { // is this at the expected phase?
            ...
            <form name="weightsform" method="post" action="submissions.php">
                ...
                <CENTER>
                    ...
                    echo "<TR><td COLSPAN=\"2\" bgcolor=\"$THEME->cellheading2\"><CENTER><B>" . get_string('weightsusedforfinalgrade', 'workshop') . "</B></CENTER></TD></TR>\n";
                    ...
                    echo "<TR><TD COLSPAN=\"2\" bgcolor=\"$THEME->cellheading2\"><CENTER><B>" . get_string('optionforpeergrade', 'workshop') . "</B></CENTER></TD></TR>\n";
                    ...
                    choose_from_menu($numbers, 'nentries', (string)$nentries, '');
                    ...
                    /******************* list all submissions ************************************/ elseif ('listallsubmissions' == $action) {
                        ...
                        print_continue("view.php?id=$cm->id");
                    } /******************* list for assessment student (submissions) ************************************/ elseif ('listforassessmentstudent' == $action) {
                        if (!$users = get_course_students($course->id)) {
                            print_heading(get_string('nostudentsyet'));
                        ...
                        print_continue("view.php?id=$cm->id");
                    } /******************* list for assessment teacher (submissions) ************************************/ elseif ('listforassessmentteacher' == $action) {
                        if (!$users = get_course_students($course->id)) {
                            print_heading(get_string('nostudentsyet'));
                        ...
                        print_continue("view.php?id=$cm->id");
                    } /******************* save analysis options (for teachers only) ************************************/ elseif ('saveanalysisoptions' == $action) {
                        if (!isteacher($course->id)) {
                            error('Only teachers can look at this page');
                        }

                        set_field('workshop', 'teacherloading', $_POST['teacherloading'], 'id', (string)$workshop->id);

                        set_field('workshop', 'gradingweight', $_POST['gradingweight'], 'id', (string)$workshop->id);

                        set_field('workshop', 'assessmentstodrop', $_POST['assessmentstodrop'], 'id', (string)$workshop->id);

                        redirect(
                            "submissions.php?id=$cm->id&action=analysisofassessments",
                            get_string('savedok', 'workshop')
                        );
                    } /*************** update (league table options teacher) ***************************/ elseif ('updateleaguetable' == $action) {
                        if (!isteacher($course->id)) {
                            error('Only teachers can look at this page');
                        }
                    ...
                    } /*************** update over allocation (by teacher) ***************************/ elseif ('updateoverallocation' == $action) {
                        if (!isteacher($course->id)) {
                            error('Only teachers can look at this page');
                        }
                        ...
                        redirect("submissions.php?action=adminlist&id=$cm->id");
                    } /******************* user confirm delete ************************************/ elseif ('userconfirmdelete' == $action) {
                        if (empty($_GET['sid'])) {
                        ...
                        notice_yesno(
                            get_string('confirmdeletionofthisitem', 'workshop', get_string('submission', 'workshop')),
                            "submissions.php?action=userdelete&id=$cm->id&sid=$_GET[sid]",
                            "view.php?id=$cm->id"
                        );
                    } /******************* user delete ************************************/ elseif ('userdelete' == $action) {
                        if (empty($_GET['sid'])) {
