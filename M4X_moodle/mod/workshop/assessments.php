...
print_header(
    "$course->shortname: $workshop->name",
...
<FORM NAME="commentform" ACTION="assessments.php" METHOD="post">
    ...
    workshop_print_assessment($workshop, $assessment);
            ...
            /******************* admin confirm delete ************************************/ elseif ('adminconfirmdelete' == $action) {
                ...
                notice_yesno(
                    get_string('confirmdeletionofthisitem', 'workshop', get_string('assessment', 'workshop')),
            ...
            } /*********************** admin list of asssessments (of a submission) (by teachers)**************/ elseif ('adminlist' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
            ...
            } /*********************** admin list of asssessments by a student (used by teachers only )******************/ elseif ('adminlistbystudent' == $action) {
                if (!isteacher($course->id)) {
                    error('Only teachers can look at this page');
                }
                ...
                print_continue("submissions.php?action=adminlist&a=$workshop->id");
            } /*************** agree (to) assessment (by student) ***************************/ elseif ('agreeassessment' == $action) {
                $timenow = time();
                ...
                add_to_log($course->id, 'workshop', 'agree', "assessments.php?action=viewassessment&id=$cm->id&aid=$assessment->id", (string)$assessment->id);

                print_continue("view.php?id=$cm->id");
            } /*************** Assess submission (by teacher or student) ***************************/ elseif ('assesssubmission' == $action) {
            ...
            } /*************** display grading form (viewed by student) *********************************/ elseif ('displaygradingform' == $action) {
            ...
            <FORM NAME="gradingform" ACTION="assessments.php" METHOD="post">
                ...
                workshop_print_assessment($workshop, $assessment);
                        ...
                        /*********************** edit assessment elements (for teachers) ***********************/ elseif ('editelements' == $action) {
                            if (!isteacher($course->id)) {
                                error('Only teachers can look at this page');
                            }
                            ...
                            switch ($workshop->gradingstrategy) {
                                            ...
                                            case 2: // error banded grading
                                                ...
                                                echo '<CENTER><TABLE cellpadding=5 border=1><TR><TD ALIGN="CENTER">' . get_string('numberofnegativeresponses', 'workshop');
                        ...
                        <FORM NAME="gradingform" ACTION="assessments.php" METHOD="post">
                            ...
                            echo "</CENTER></FORM>\n";
                                    ...
                                    /*************** insert (new) comment (by author, assessor or teacher) ***************************/ elseif ('insertcomment' == $action) {
                                        $timenow = time();
                                        ...
                                        add_to_log($course->id, 'workshop', 'comment', "view.php?id=$cm->id", (string)$comment->id);

                                        print_continue("assessments.php?action=viewassessment&id=$cm->id&aid=$assessment->id");
                                    } /*********************** insert/update assignment elements (for teachers)***********************/ elseif ('insertelements' == $action) {
                                        if (!isteacher($course->id)) {
                                            error('Only teachers can look at this page');
                                        }
                                                    ...
                                                    if ($description) {
                                                        ...
                                                        if (isset($form->scale[$key])) {
                                                            ...
                                                            switch ($WORKSHOP_SCALES[$form->scale[$key]]['type']) {
                                                                case 'radio':
                                                                    $element->maxscore = $WORKSHOP_SCALES[$form->scale[$key]]['size'] - 1;
                                                                    break;
                                                                case 'selection':
                                                                    $element->maxscore = $WORKSHOP_SCALES[$form->scale[$key]]['size'];
                                                                    break;
                                                            }
                                                        }
                                    ...
                                    } /*********************** list assessments for grading (Student submissions)(by teachers)***********************/ elseif ('listungradedstudentsubmissions' == $action) {
                                        if (!isteacher($course->id)) {
                                            error('Only teachers can look at this page');
                                        }
                                        ...
                                        print_continue("view.php?a=$workshop->id");
                                    } /*********************** list assessments for grading (Teacher submissions) (by teachers)***********************/ elseif ('listungradedteachersubmissions' == $action) {
                                        if (!isteacher($course->id)) {
                                            error('Only teachers can look at this page');
                                        }
                                        ...
                                        print_continue("view.php?a=$workshop->id");
                                    } /****************** list teacher submissions ***********************/ elseif ('listteachersubmissions' == $action) {
                                        workshop_list_teacher_submissions($workshop, $USER);

                                        print_continue("view.php?a=$workshop->id");
                                    } /*************** update assessment (by teacher or student) ***************************/ elseif ('updateassessment' == $action) {
                                        ...
                                        switch ($workshop->gradingstrategy) {
                                            ...
                                            case 2: // error banded graded
                                                ...
                                                $grade = ($elements[(int)($error + 0.5)]->maxscore + $form->grade[$i]);
                                                echo '<P><B>' . get_string('weightederrorcount', 'workshop', (int)($error + 0.5)) . "</B>\n";
                                                break;
                                        ...
                                        add_to_log(
                                            ...
                                            'workshop',
                                            'assess',
                                            "assessments.php?action=viewassessment&id=$cm->id&aid=$assessment->id",
                                        ...
                                        if ($workshop->gradingstrategy) {
                                            redirect(
                                                $returnto,
                                                get_string('thegradeis', 'workshop') . ': ' . number_format($grade, 2) . ' (' . get_string('maximumgrade') . ' ' . number_format($workshop->grade) . ')'
                                            );
                                        } else {
                                            redirect($returnto);
                                        }
                                    } /****************** update comment (by author, assessor or teacher) ********************/ elseif ('updatecomment' == $action) {
                                        $timenow = time();
                                        ...
                                        print_continue("assessments.php?action=viewassessment&id=$cm->id&aid=$assessment->id");
                                    } /****************** update grading (by teacher) ***************************/ elseif ('updategrading' == $action) {
                                        $timenow = time();
                                        ...
                                        notice_yesno(
                                            ...
                                            "assessments.php?action=userdelete&id=$cm->id&aid=$_GET[aid]",
                                            "view.php?id=$cm->id"
                                        );
                                        ...
                                        print_continue("view.php?id=$cm->id");
                                    } /****************** view all assessments ***********************/ elseif ('viewallassessments' == $action) {
                                    ...
                                    } /****************** view assessment *****************************/ elseif ('viewassessment' == $action) {
                                        optional_variable($allowcomments);
