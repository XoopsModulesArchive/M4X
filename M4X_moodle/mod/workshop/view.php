...
print_header_simple(
    ...
    '',
    true,
    update_module_button($cm->id, $course->id, $strworkshop),
    navmenu($course, $cm)
);
...
if ('allowassessments' == $action) {
    ...
    set_field('workshop', 'phase', 4, 'id', (string)$workshop->id);

    add_to_log($course->id, 'workshop', 'assessments only', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);
...
if ('allowboth' == $action) {
    ...
    set_field('workshop', 'phase', 3, 'id', (string)$workshop->id);

    add_to_log($course->id, 'workshop', 'allow both', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);

    redirect("view.php?a=$workshop->id", get_string('movingtophase', 'workshop', 3));
}

/************** allow submissions only (move to phase 2) (for teachers)**/
if ('allowsubmissions' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }
    ...
    if (workshop_count_teacher_submissions($workshop) < $workshop->ntassessments) {
        redirect(
            "view.php?id=$cm->id",
            get_string(
                'notenoughexamplessubmitted',
                'workshop',
                $course->teacher
            )
        );
    } else {
        set_field('workshop', 'phase', 2, 'id', (string)$workshop->id);

        add_to_log($course->id, 'workshop', 'submissions', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);

        redirect("view.php?id=$cm->id", get_string('movingtophase', 'workshop', 2));
    }
} /****************** close workshop for student assessments/submissions (move to phase 5) (for teachers)**/ elseif ('closeworkshop' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }
    ...
    set_field('workshop', 'phase', 5, 'id', (string)$workshop->id);

    add_to_log($course->id, 'workshop', 'close', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);

    redirect("view.php?a=$workshop->id", get_string('movingtophase', 'workshop', 5));
} /****************** display final grade (for students) ************************************/ elseif ('displayfinalgrade' == $action) {
    ...
    if ($submissions = workshop_get_user_submissions($workshop, $USER)) { // any submissions from user?
        ...
        if ($useteachersgrades) {
            echo '<td align="center"><b>' . get_string(
                'teacherassessments',
                'workshop',
                $course->teacher
            ) . '</b></td>';
        ...
        foreach ($submissions as $submission) {
            ...
            if ($useteachersgrades) {
                echo '<td align="center">' . workshop_print_submission_assessments(
                    $workshop,
                    $submission,
                    'teacher'
                ) . '</td>';
...
} /****************** make final grades available (go to phase 6) (for teachers only)********/ elseif ('makefinalgradesavailable' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }

    set_field('workshop', 'phase', 6, 'id', (string)$workshop->id);

    add_to_log($course->id, 'workshop', 'display grades', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);

    redirect("view.php?a=$workshop->id", get_string('movingtophase', 'workshop', 6));
} /****************** assignment not available (for students)***********************/ elseif ('notavailable' == $action) {
    print_heading(get_string('notavailable', 'workshop'));
} /****************** set up assignment (move back to phase 1) (for teachers)***********************/ elseif ('setupassignment' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }

    set_field('workshop', 'phase', 1, 'id', (string)$workshop->id);

    add_to_log($course->id, 'workshop', 'set up', "view.php?id=$cm->id", (string)$workshop->id, $cm->id);

    redirect("view.php?a=$workshop->id", get_string('movingtophase', 'workshop', 1));
} /****************** student's view could be in 1 of 4 stages ***********************/ elseif ('studentsview' == $action) {
    ...
    if ($workshop->ntassessments and (!workshop_test_user_assessments($workshop, $USER))) {
        print_heading(
            get_string(
                'pleaseassesstheseexamplesfromtheteacher',
                'workshop',
                $course->teacher
            )
        );
        ...
        if ($workshop->ntassessments) {
            ...
            print_heading(
                get_string(
                    'yourassessmentsofexamplesfromtheteacher',
                    'workshop',
                    $course->teacher
                )
            );
...
} /****************** submission of assignment by teacher only***********************/ elseif ('submitassignment' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }
    ...
    workshop_print_upload_form($workshop);
} /****************** teacher's view - display admin page (current phase options) ************/ elseif ('teachersview' == $action) {
    if (!isteacher($course->id)) {
        error('Only teachers can look at this page');
    }
    ...
    $tabs->urls = [
        "view.php?id=$cm->id&action=setupassignment",
    ...
    switch ($workshop->phase) {
        ...
        case 1: // set up assignment
            if ($workshop->nelements) {
                echo "<p><b><a href=\"assessments.php?id=$cm->id&action=editelements\">" . get_string('amendassessmentelements', 'workshop') . "</a></b> \n";
            ...
            if ($workshop->ntassessments) {
                ...
                echo "<p><b><a href=\"view.php?id=$cm->id&action=submitassignment\">" . get_string('submitexampleassignment', 'workshop') . "</a></b> \n";
                ...
                echo "<p><b><a href=\"submissions.php?id=$cm->id&action=listforassessmentteacher\">" . get_string(
                    'teachersubmissionsforassessment',
                    'workshop',
        ...
        case 4:
            if ($workshop->ntassessments) { // if teacher example show student assessments link
                echo "<p><b><a href=\"assessments.php?id=$cm->id&action=listungradedteachersubmissions\">" . get_string(
                    'ungradedassessmentsofteachersubmissions',
                    'workshop',
                    workshop_count_ungraded_assessments_teacher($workshop)
                ) . "</a></b> \n";

                helpbutton(
                    'ungradedassessments_teacher',
                    get_string('ungradedassessmentsofteachersubmissions', 'workshop'),
                    'workshop'
                );
            }
            echo "<p><b><a href=\"assessments.php?id=$cm->id&action=listungradedstudentsubmissions\">" . get_string(
                'ungradedassessmentsofstudentsubmissions',
                'workshop',
                workshop_count_ungraded_assessments_student($workshop)
            ) . "</a></b> \n";
            helpbutton(
                'ungradedassessments_student',
                get_string('ungradedassessmentsofstudentsubmissions', 'workshop'),
                'workshop'
            );
            echo "<p><b><a href=\"submissions.php?id=$cm->id&action=listforassessmentstudent\">" . get_string(
                'studentsubmissionsforassessment',
                'workshop',
        ...
        case 5: // calculate final grades
            ...
            print_heading(
                "<a href=\"submissions.php?id=$cm->id&action=displayfinalweights\">" . get_string('calculationoffinalgrades', 'workshop') . '</a>'
            );
            print_heading(
                "<a href=\"submissions.php?id=$cm->id&action=analysisofassessments\">" . get_string('analysisofassessments', 'workshop') . '</a>'
            );
            break;
        case 6: // show final grades
            print_heading(
                "<A HREF=\"submissions.php?id=$cm->id&action=displayfinalgrades\">" . get_string('displayoffinalgrades', 'workshop') . '</A>'
            );
    }

    print_heading(
        "<A HREF=\"submissions.php?id=$cm->id&action=adminlist\">" . get_string('administration') . '</A>'
    );
} /*************** no man's land **************************************/ else {
