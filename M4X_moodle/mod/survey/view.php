...
// $Id: view.php,v 1.27 2004/08/21 20:20:57 gustav_delius Exp $
...
print_header_simple(
    (string)$survey->name,
    '',
    "<A HREF=index.php?id=$course->id>$strsurveys</A> -> $survey->name",
    '',
    '',
    true,
...
if (isteacheredit($course->id) or (VISIBLEGROUPS == $groupmode)) {
    $currentgroup = 0;
}
...
if (isteacher($course->id)) {
    ...
    echo "<p align=right><a href=\"report.php?id=$cm->id\">" . get_string('viewsurveyresponses', 'survey', $numusers) . '</A></P>';
} elseif (!$cm->visible) {
