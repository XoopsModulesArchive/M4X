...
// $Id: report.php,v 1.27 2004/03/20 09:39:17 moodler Exp $
...
$showscales = ('ciqname' != $template->name);
...
add_to_log($course->id, 'survey', 'view report', "report.php?id=$cm->id", (string)$survey->id, $cm->id);
...
print_header(
    "$course->shortname: $survey->name",
    (string)$course->fullname,
    "$navigation $strreport",
    '',
    '',
    true,
...
if ($groupmode = groupmode($course, $cm)) {   // Groups are being used
    $menuaction = 'student' == $action ? 'students' : $action;

    $currentgroup = setup_and_print_groups(
        $course,
        $groupmode,
...
switch ($action) {
    ...
    case 'questions':
        ...
        if ($qid) {     // just get one multi-question
            ...
            if ($scale = get_records('survey_questions', 'multi', (string)$qid)) {
    ...
    case 'question':
        ...
        $table = null;
        $table->head = ['', $strname, $strtime, $stractual, $strpreferred];
    ...
    case 'students':
        ...
        print_heading(get_string('analysisof', 'survey', (string)$course->students));
    ...
    case 'student':
        ...
        foreach ($questionorder as $key => $val) {
            ...
            if (0 == $question->type or 1 == $question->type) {
                if ($answer = survey_get_user_answer($survey->id, $question->id, $user->id)) {
                    $table = null;

                    $table->head = [get_string($question->text, 'survey')];

                    $table->align = ['left'];

                    $table->data[] = [(string)$answer->answer1];
