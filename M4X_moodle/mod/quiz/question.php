...
// $Id: question.php,v 1.44 2004/08/22 19:21:02 gustav_delius Exp $
...
} elseif (!isset($QUIZ_QTYPES[$qtype])) {
...
print_header_simple(
    (string)$streditingquestion,
    (string)$streditingquestion,
    "<a href=\"$CFG->wwwroot/mod/quiz/index.php?id=$course->id\">$strquizzes</a>" . " -> <a href=\"edit.php\">$streditingquiz</a> -> $streditingquestion"
);
...
if (isset($delete)) {
    if (isset($confirm)) {
        ...
        if ($beingused) {
            ...
            notice_yesno(
                get_string('deletequestioncheck', 'quiz', $question->name),
                "question.php?id=$question->id&delete=$delete&confirm=" . md5($delete),
                'edit.php'
            );
        }
...
$grades = [1, 0.9, 0.8, 0.75, 0.70, 0.66666, 0.60, 0.50, 0.40, 0.33333, 0.30, 0.25, 0.20, 0.16666, 0.10, 0.05, 0];
foreach ($grades as $grade) {
    $percentage = 100 * $grade;

    $neggrade = -$grade;

    $gradeoptions[(string)$grade] = "$percentage %";

    $gradeoptionsfull[(string)$grade] = "$percentage %";

    $gradeoptionsfull[(string)$neggrade] = -$percentage . ' %';
}
...
foreach ($coursefiles as $filename) {
    if ('image.gif' == mimeinfo('icon', $filename)) {
        $images[(string)$filename] = $filename;
    }
}
...
require __DIR__ . '/questiontypes/' . $QUIZ_QTYPES[$qtype]->name() . '/editquestion.php';
