...
// $Id: category.php,v 1.13 2004/08/22 14:38:43 gustav_delius Exp $
...
$streditingquiz = get_string(
    isset($modform->instance) ? 'editingquiz' : 'editquestions',
    'quiz'
);
$streditcategories = get_string('editcategories', 'quiz');

print_header_simple(
    (string)$streditcategories,
    " $streditcategories",
...
if ($form = data_submitted()) {
    ...
    foreach ($form as $key => $val) {
        ...
        } elseif ('c' == mb_substr($key, 0, 1)) {
            $cat->id = mb_substr($key, 1);
...
$table->head = [$strcategory, $strcategoryinfo, $strpublish, $strquestions, $straction];
$table->align = ['LEFT', 'LEFT', 'CENTER', 'CENTER', 'CENTER'];
$table->size = ['80', '80', '40', '40', '50'];
$table->width = 200;
$table->nowrap = true;
...
foreach ($categories as $category) {
    ...
    $table->data[] = [
        "<INPUT TYPE=text NAME=\"c$category->id\" VALUE=\"$category->name\" SIZE=15>",
        "<INPUT TYPE=text NAME=\"i$category->id\" VALUE=\"$category->info\" SIZE=50>",
        choose_from_menu($publishoptions, "p$category->id", (string)$category->publish, '', '', '', true),
        (string)$count,
        $delete,
    ];
}
$table->data[] = [
    '<INPUT TYPE=text NAME="new" VALUE="" SIZE=15>',
