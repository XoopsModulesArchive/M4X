<?PHP // $Id: lib.php,v 1.175.2.3 2004/11/10 14:05:01 moodler Exp $
...
class quiz_default_questiontype
{
    ...
    publicfunction uses_quizfile($question, $relativefilepath)
    {
        // The default does only check whether the file is used as image:
        return $question->image == $relativefilepath;
    }

    publicfunction save_question_options($question)
    {
    ...
    publicfunction save_question($question, $form, $course)
    {
    ...
    publicfunction extract_response_id($responsekey)
    {
        if (preg_match('[0-9]' . $this->name() . '([0-9]+)', $responsekey, $regs)) {
            return $regs[1];
        } else {
            return false;
        }
    ...
    publicfunction wrapped_questions($question)
    {
    ...
    publicfunction convert_to_response_answer_field($questionresponse)
    {
    ...
    publicfunction get_answers($question)
    {
    ...
    publicfunction create_response($question, $nameprefix, $questionsinuse)
    {
    ...
    publicfunction extract_response($rawresponse, $nameprefix)
    {
        ...
        return [$nameprefix => $rawresponse->answer];
    }

    publicfunction print_question_number_and_grading_details($number, $grade, $actualgrade = false, $recentlyadded = false)
    {
    ...
    publicfunction print_question_formulation_and_controls(
        $question,
        $quiz,
        $readonly,
        $answers,
        ...
        notify(
            'Error: Question formulation and input controls has not' . '  been implemented for question type ' . $this->name()
        );
    }

    publicfunction grade_response($question, $nameprefix)
    {
        ...
        error(
            'grade_response has not been implemented for question type ' . $this->name()
        );
    }
}
...
function quiz_delete_instance($id)
{
    ...
    if (!$quiz = get_record('quiz', 'id', (string) $id)) {
        return false;
    }

    $result = true;

    if ($attempts = get_records('quiz_attempts', 'quiz', (string) $quiz->id)) {
        foreach ($attempts as $attempt) {
            if (!delete_records('quiz_responses', 'attempt', (string) $attempt->id)) {
                $result = false;
            }
        }
    }

    if (!delete_records('quiz_attempts', 'quiz', (string) $quiz->id)) {
        $result = false;
    }

    if (!delete_records('quiz_grades', 'quiz', (string) $quiz->id)) {
        $result = false;
    }

    if (!delete_records('quiz_question_grades', 'quiz', (string) $quiz->id)) {
        $result = false;
    }

    if (!delete_records('quiz', 'id', (string) $quiz->id)) {
        $result = false;
    }
...
function quiz_grades($quizid)
{
    ...
    $return->maxgrade = get_field('quiz', 'grade', 'id', (string) $quizid);
    return $return;
}
...
function quiz_move_questions($category1, $category2)
{
    global $CFG;
    return execute_sql(
        "UPDATE {$CFG->prefix}quiz_questions
...
function quiz_get_grade_records($quiz)
{
    ...
    return get_records_sql(
        "SELECT qg.*, u.firstname, u.lastname, u.picture
...
function quiz_get_attempt_questions($quiz, $attempt, $attempting = false)
{
    ...
    if (!($rawresponses = get_records_sql(
            "SELECT question, answer, attempt FROM {$CFG->prefix}quiz_responses
               ...
               // Try to get responses from the previous attempt: and $lastattemptnum = $attempt->attempt - 1) {
        {do {
            $lastattempt = get_record(
                'quiz_attempts',
                'quiz',
                $quiz->id,
                'userid',
        ...
        if (0 == $lastattemptnum or !($rawresponses = get_records_sql(
    ...
    foreach ($questions as $qid => $question) {
        ...
        if (isset($rawresponses[$qid])) {
            $questions[$qid]->response      = $QUIZ_QTYPES[$question->qtype]->extract_response(
                    $rawresponses[$qid],
                    quiz_qtype_nameprefix($question)
                );
    ...
    if ($attempting) {
        ...
        foreach ($questions as $question) {
            if ($wrapped = $QUIZ_QTYPES[$question->qtype]->wrapped_questions($question, quiz_qtype_nameprefix($question))) {
        ...
        foreach ($questions as $question) {
            if (empty($question->response)) {
                ...
                $questions[$question->id]->response = $QUIZ_QTYPES[$question->qtype]->create_response($question, $nameprefix, $questionsinuse);
...
{
    if (preg_match("^$nameprefix([0-9]+)", $name, $regs)) {
        return $regs[1];
    } else {
        return false;
    }
...
function quiz_print_possible_question_image($quizid, $question)
{
    ...
    if ($question->image) {
        ...
        if (substr(strtolower($question->image), 0, 7) == 'http://') {
            echo $question->image;
        } elseif ($CFG->slasharguments) {        // Use this method if possible for better caching
...
function quiz_print_quiz_questions(
    ...
    $readonly = !empty($results) && !isset($results->attemptbuildsonthelast) ? ' readonly="readonly" ' : '';
    ...
    foreach ($questions as $question) {
        ...
        $nextquestionnumber = $QUIZ_QTYPES[$question->qtype]->print_question($nextquestionnumber, $quiz, $question, $readonly, $details);
...
function quiz_print_question_list($questionlist, $grades)
{
    ...
    foreach ($order as $qnum) {
        ...
        if ($question->qtype == DESCRIPTION) {
            ...
            choose_from_menu(
                quiz_gradesmenu_options($question->defaultgrade),
                "q$qnum",
                (string)$grades[$qnum],
                ''
            );
...
function quiz_print_cat_question_list($categoryid, $quizselected = true)
{
    ...
    if (!$category = get_record('quiz_categories', 'id', (string) $categoryid)) {
    ...
    if (isteacheredit($category->course)) {
        ...
        popup_form(
            "question.php?category=$category->id&qtype=",
            $QUIZ_QUESTION_TYPE,
            'addquestion',
            '',
            'choose',
...
function quiz_save_attempt($quiz, $questions, $result, $attemptnum)
{
    ...
    foreach ($questions as $question) {
        ...
        if (!empty($question->response)) {
            $response->answer = $QUIZ_QTYPES[$question->qtype]->convert_to_response_answer_field($question->response);

            ///////////////////////////////////////////
            // WORKAROUND for question type RANDOM:
            ///////////////////////////////////////////
            if ($question->qtype == RANDOM and preg_match('^random([0-9]+)-(.*)$', $response->answer, $afields)) {
...
function quiz_extract_correctanswers($answers, $nameprefix)
{
    ...
    foreach ($answers as $answer) {
        ...
        } elseif ($answer->fraction == $bestanswerfraction) {
            $correctanswers[$nameprefix . $answer->id] = $answer;
        }
    }
...
function quiz_grade_responses($quiz, $questions)
{
    ...
    foreach ($questions as $qid => $question) {
        ...
        } elseif (((float)($resultdetails->grade)) >= 1.0) {
...
function quiz_get_recent_mod_activity(&$activities, &$index, $sincetime, $courseid, $quiz = '0', $user = '', $groupid = '')
{
    ...
    $quizzes = get_records_sql(
        "SELECT qa.*, q.name, u.firstname, u.lastname, u.picture,
...
function quiz_print_recent_mod_activity($activity, $course, $detail = false)
{
    ...
    if ($detail) {
        echo "<img src=\"$CFG->modpixpath/$activity->type/icon.gif\" " . "height=16 width=16 alt=\"$activity->type\">  ";
        echo "<a href=\"$CFG->wwwroot/mod/quiz/view.php?id=" . $activity->instance . '">' . $activity->name . '</a> - ';
    }

    if (isteacher($USER)) {
        $grades = '(' . $activity->content->sumgrades . ' / ' . $activity->content->maxgrade . ') ';
        echo "<a href=\"$CFG->wwwroot/mod/quiz/review.php?q=" . $activity->instance . '&attempt=' . $activity->content->attemptid . '">' . $grades . '</a> ';
