...
// $Id: attempt.php,v 1.44.2.1 2004/11/11 02:21:06 moodler Exp $
...
if ($quiz->password and empty($_POST['q'])) {
    ...
    if (0 !== strcmp($quiz->password, $_POST['quizpassword'])) {
...
if ($rawanswers = data_submitted()) {
    ...
    if ($attempt = quiz_save_attempt($quiz, $questions, $result, $attemptnumber)) {
        add_to_log(
            ...
            'quiz',
            'submit',
            "review.php?id=$cm->id&attempt=$attempt->id",
            (string)$quiz->id,
            $cm->id
        );
    } else {
...
if (isset($unattempt) && $unattempt) {
    $attempt = $unattempt;
} elseif ($attempt = quiz_start_attempt($quiz->id, $USER->id, $attemptnumber)) {
    add_to_log(
        $course->id,
        'quiz',
        'attempt',
...
if ($quiz->attemptonlast && $attemptnumber >= 2 and 0 == $quiz->attempts || !unattempt) {
