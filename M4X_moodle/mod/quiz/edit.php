...
// $Id: edit.php,v 1.29 2004/08/23 12:29:29 moodler Exp $
...
} elseif ($courseid) { // Page retrieve through "Edit Questions" link - no quiz selected
...
if (!empty($add)) { /// Add a question to the current quiz
    ...
    foreach ($rawquestions as $key => $value) {    // Parse input for question ids
        if ('q' == mb_substr($key, 0, 1)) {
            ...
            } elseif (DESCRIPTION == $questionrecord->qtype) {
...
if (!isset($modform->instance)) {
    ...
    <center>
        ...
        if ($attemptcount = count_records_select('quiz_attempts', "quiz = '$modform->instance' AND timefinish > 0")) {
            ...
            $usercount = count_records('quiz_grades', 'quiz', (string)$modform->instance);
