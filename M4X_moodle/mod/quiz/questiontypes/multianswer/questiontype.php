...
// $Id: questiontype.php,v 1.3 2004/08/04 14:09:19 kaipe Exp $
...
class quiz_embedded_cloze_qtype extends quiz_default_questiontype
{
    public function get_answers($question)
    {
        ...
        if ($multianswers = get_records('quiz_multianswers', 'question', $question->id)) {
            foreach ($multianswers as $multianswer) {
                ...
                $multianswer->subanswers = $QUIZ_QTYPES[$multianswer->answertype]->get_answers($virtualquestion, $addedcondition);
    ...
    public function get_position_multianswer($questionid, $positionkey)
    {
    ...
    public function get_multianswers($questionid)
    {
        // As a separate function in order to make it overridable
    ...
    public function save_question_options($question)
    {
        ...
        foreach ($question->answers as $dataanswer) {
            if ($oldmultianswer = array_shift($oldmultianswers)) {  // Existing answer, so reuse it
                ...
                if (!$multianswer->answers = quiz_qtype_multianswer_save_alternatives(
                    $question->id,
    ...
    public function save_question($authorizedquestion, $form, $course)
    {
    ...
    public function convert_to_response_answer_field($questionresponse)
    {
    ...
    public function extract_response($rawresponse, $nameprefix)
    {
        ...
        foreach ($multianswers as $maid => $multianswer) {
            if (preg_match("(^|,)$maid-(.*)", $rawresponse->answer, $regs)) {
                $splits = preg_split(',[0-9]+-', $regs[2], 2);
    ...
    public function print_question_formulation_and_controls(
        $question,
        $quiz,
        $readonly,
        $answers,
        ...
        while (preg_match('\{#([^[:space:]}]*)}', $qtextremaining, $regs)) {
            ...
            $response = $question->response[$inputname] ?? '';
            ...
            if (!empty($correctanswers) && '' !== $response) {
                ...
                } elseif ($answers[$inputname]->fraction >= 1.0) {
            ...
            if ($quiz->feedback && isset($answers[$inputname])
                ...
                $popup = " onmouseover=\"return overlib('$title', CAPTION, '$strfeedback', FGCOLOR, '$THEME->cellcontent');\" " . ' onmouseout="return nd();" ';
            } else {
                $popup = '';
            }
            ...
            switch ($multianswer->answertype) {
                ...
                case MULTICHOICE:
                    ...
                    foreach ($mcanswers as $mcanswer) {
                        $selected = $response == $mcanswer->id ? ' selected="selected" ' : '';
    ...
    public function grade_response($question, $nameprefix)
    {
        global $QUIZ_QTYPES;
        ...
        foreach ($multianswers as $multianswer) {
            ...
            $subquestion->response[$nameprefix] = $question->response[$name] ?? '';

            $subresult = $QUIZ_QTYPES[$multianswer->answertype]->grade_response(
                $subquestion,
                $nameprefix,
...
function quiz_qtype_multianswer_extract_question($text)
{
    ...
    define(
        ...
        '(' . ANSWER_ALTERNATIVE_FRACTION_REGEX . ')?' . '(' . ANSWER_ALTERNATIVE_ANSWER_REGEX . ')' . '(#(' . ANSWER_ALTERNATIVE_FEEDBACK_REGEX . '))?'
    );
    ...
    define(
        'ANSWER_REGEX',
        ANSWER_START_REGEX . '(' . ANSWER_ALTERNATIVE_REGEX . '(~' . ANSWER_ALTERNATIVE_REGEX . ')*)}'
    );
    ...
    for (
        $positionkey = 1; preg_match(ANSWER_REGEX, $question->questiontext, $answerregs); ++$positionkey
    ) {
        unset($multianswer);

        $multianswer->positionkey = $positionkey;

        $multianswer->norm = $answerregs[ANSWER_REGEX_NORM] or $multianswer->norm = '1';

        if ($answerregs[ANSWER_REGEX_ANSWER_TYPE_NUMERICAL]) {
            $multianswer->answertype = NUMERICAL;
        } elseif ($answerregs[ANSWER_REGEX_ANSWER_TYPE_SHORTANSWER]) {
            $multianswer->answertype = SHORTANSWER;
        } elseif ($answerregs[ANSWER_REGEX_ANSWER_TYPE_MULTICHOICE]) {
        ...
        while (preg_match(ANSWER_ALTERNATIVE_REGEX, $remainingalts, $altregs)) {
            ...
            if ('=' == $altregs[ANSWER_ALTERNATIVE_REGEX_FRACTION]) {
                ...
                $alternative->fraction = .01 * $altregs[ANSWER_ALTERNATIVE_REGEX_PERCENTILE_FRACTION] or $alternative->fraction = '0';
            }
            ...
            if ($answerregs[ANSWER_REGEX_ANSWER_TYPE_NUMERICAL]
                ...
                if ($numregs[NUMERICAL_ABS_ERROR_MARGIN]) {
                    $alternative->min = $numregs[NUMERICAL_CORRECT_ANSWER] - $numregs[NUMERICAL_ABS_ERROR_MARGIN];

                    $alternative->max = $numregs[NUMERICAL_CORRECT_ANSWER] + $numregs[NUMERICAL_ABS_ERROR_MARGIN];
                } else {
                ...
                $alternative->answer = $altregs[ANSWER_ALTERNATIVE_REGEX_ANSWER];
            }
...
function quiz_qtype_multianswer_save_alternatives(
    $questionid,
    $answertype,
    $alternatives,
    $oldalternativeids = null
) {
    // Returns false if something goes wrong,

    // otherwise the ids of the answers.

    if (empty($oldalternativeids) or !($oldalternatives = get_records_list('quiz_answers', 'id', $oldalternativeids))) {
        $oldalternatives = [];
    }
    ...
    foreach ($alternatives as $altdata) {
        ...
        if (NUMERICAL == $answertype) {
            if ($numericaloptions = get_record('quiz_numerical', 'answer', $alt->id)) {
    ...
    switch ($answertype) {
        ...
        case SHORTANSWER:
            ...
            } elseif ($answers != $oldalternativeids) {
