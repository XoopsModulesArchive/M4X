...
// $Id: review.php,v 1.12 2004/08/21 20:20:57 gustav_delius Exp $
...
add_to_log($course->id, 'quiz', 'review', "review.php?id=$cm->id&attempt=$attempt->id", (string)$quiz->id, (string)$cm->id);
...
$table->data[] = ["$strtimetaken:", $timetaken];
$table->data[] = ["$strtimecompleted:", userdate($attempt->timefinish)];
if (!empty($overtime)) {
    $table->data[] = ["$stroverdue:", $overtime];
}
if ($quiz->grade) {
    if ($overtime) {
        $result->sumgrades = '0';
    ...
    $table->data[] = ["$strscore:", "$result->sumgrades/$quiz->sumgrades ($result->percentage %)"];

    $table->data[] = ["$strgrade:", "$result->grade/$quiz->grade"];
}
