...
// $Id: view.php,v 1.16.2.2 2004/09/19 14:00:00 moodler Exp $
...
print_header_simple(
    ...
    '',
    true,
    update_module_button($cm->id, $course->id, $strlesson),
    navmenu($course, $cm)
);
...
if ('navigation' == $action) {
    ...
    if (empty($pageid)) {
        add_to_log($course->id, 'lesson', 'start', "view.php?id=$cm->id", (string)$lesson->id, $cm->id);

        // if no pageid given see if the lesson has been started

        if ($grades = get_records_select(
            'lesson_grades',
            "lessonid = $lesson->id AND userid = $USER->id",
            'grade DESC'
        )) {
        ...
        if ($attempts = get_records_select(
            'lesson_attempts',
            ...
            foreach ($attempts as $attempt) {
                ...
                if (0 == $jumpto) { // unlikely value!
                    $lastpageseen = $attempt->pageid;
                } elseif (LESSON_NEXTPAGE == $jumpto) {
                    if (!$lastpageseen = get_field(
                        'lesson_pages',
                        'nextpageid',
                        'id',
                        $attempt->pageid
            ...
            if ($lastpageseen != $firstpageid) {
                ...
                print_simple_box(
                    '<p align="center">' . get_string('youhaveseen', 'lesson') . '</p>',
                    'center'
                );

                echo '<p align="center"><input type="button" value="'
                     . get_string('yes')
                     . "\" onclick=\"document.queryform.pageid.value='$lastpageseen';document.queryform.submit();\">&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\""
                     . get_string('no')
    ...
    if (LESSON_EOL != $pageid) {
        add_to_log($course->id, 'lesson', 'view', "view.php?id=$cm->id", (string)$pageid, $cm->id);
        ...
        if (LESSON_ENDOFBRANCH == $page->qtype) {
            if ($answers = get_records('lesson_answers', 'pageid', $page->id, 'id')) {
                ...
                foreach ($answers as $answer) {
                    ...
                    redirect(
                        "view.php?id=$cm->id&action=navigation&pageid=$answer->jumpto",
        ...
        if (LESSON_BRANCHTABLE == $page->qtype) {
            if ($lesson->minquestions and isstudent($course->id)) {
                ...
                if ($nviewed) {
                    echo '<p align="center">' . get_string('numberofpagesviewed', 'lesson', $nviewed) . '; (' . get_string('youshouldview', 'lesson', $lesson->minquestions) . ')<br>';

                    // count the number of distinct correct pages

                    if ($correctpages = get_records_select(
                        'lesson_attempts',
                        "lessonid = $lesson->id
                    ...
                    $thegrade = (int)(100 * $ncorrect / $nviewed);
        ...
        if ($answers = get_records('lesson_answers', 'pageid', $page->id, 'id')) {
            ...
            switch ($page->qtype) {
                ...
                case LESSON_NUMERICAL:
                    echo '<tr><td align="center">' . get_string('youranswer', 'lesson') . ": <input type=\"text\" name=\"answer\" size=\"50\" maxlength=\"200\">\n";
                    echo '</table>';
                    print_simple_box_end();
                    echo '<p align="center"><input type="submit" name="continue" value="' . get_string('pleaseenteryouranswerinthebox', 'lesson') . "\"></p>\n";
                    break;
                case LESSON_TRUEFALSE:
                    ...
                    echo '</table>';
                    print_simple_box_end();
                    echo '<p align="center"><input type="submit" name="continue" value="' . get_string('pleasecheckoneanswer', 'lesson') . "\"></p>\n";
                    break;
                case LESSON_MULTICHOICE:
                    ...
                    print_simple_box_end();
                    if ($page->qoption) {
                        echo '<p align="center"><input type="submit" name="continue" value="' . get_string('pleasecheckoneormoreanswers', 'lesson') . "\"></p>\n";
                    } else {
                        echo '<p align="center"><input type="submit" name="continue" value="' . get_string('pleasecheckoneanswer', 'lesson') . "\"></p>\n";
                    }
                    break;
                case LESSON_MATCHING:
                    ...
                    echo '</table></table>';
                    print_simple_box_end();
                    echo '<p align="center"><input type="submit" name="continue" value="' . get_string('pleasematchtheabovepairs', 'lesson') . "\"></p>\n";
                    break;
            ...
            if ($lesson->nextpagedefault) {
                ...
                if (LESSON_UNSEENPAGE == $lesson->nextpagedefault) {
                ...
                } elseif (LESSON_UNANSWEREDPAGE == $lesson->nextpagedefault) {
                    foreach ($allpages as $thispage) {
                        if (!count_records_select(
                            'lesson_attempts',
                            "pageid = $thispage->id AND
            ...
            echo '<p align="center"><input type="submit" name="continue" value="' . get_string('continue', 'lesson') . "\"></p>\n";
        ...
        add_to_log($course->id, 'lesson', 'end', "view.php?id=$cm->id", (string)$lesson->id, $cm->id);
            ...
            if ($nviewed = count_records(
                ...
                if ($lesson->minquestions) {
                    if ($nviewed < $lesson->minquestions) {
                        ...
                        echo '<p align="center">' . get_string(
                            'youshouldview',
                            'lesson',
                            $lesson->minquestions
                        ) . ' ' . get_string('pages', 'lesson') . "</p>\n";

                        $nviewed = $lesson->minquestions;
                    }
                }

                echo '<p align="center">' . get_string('numberofcorrectanswers', 'lesson', $ncorrect) . "</p>\n";
...
} /*******************teacher view **************************************/ elseif ('teacherview' == $action) {
    ...
    if (!$page = get_record_select('lesson_pages', "lessonid = $lesson->id AND prevpageid = 0")) {
            ...
            <center>
                <table cellpadding=5 border=1>
                    <tr>
                        <td align="center">
                    <tr valign="top">
                        <td><p><b><?php print_string('pagetitle', 'lesson'); ?>:</b></p></td>
                    </tr>
                    <tr>
                        <td><input type="text" name="title" size="80" maxsize="255" value=""></td>
                    </tr>
                    <?php
                    echo '<tr><td><b>';
        ...
        // show import link
        print_heading(
            "<a href=\"import.php?id=$cm->id&pageid=0\">" . get_string(
                'importquestions',
                'lesson'
            ) . "</a>\n"
        );
        ...
        while (true) {
            ...
            if (isteacheredit($course->id)) {
                ...
                echo '<a title="'
                     . get_string('update')
                     . "\" href=\"lesson.php?id=$cm->id&action=editpage&pageid=$page->id\">\n"
                     . "<img src=\"$pixpath/t/edit.gif\" hspace=\"2\" height=11 width=11 border=0></a>\n"
                     . '<a title="'
                     . get_string('delete')
                     . "\" href=\"lesson.php?id=$cm->id&action=confirmdelete&pageid=$page->id\">\n"
            ...
            if ($answers = get_records('lesson_answers', 'pageid', $page->id, 'id')) {
                ...
                foreach ($answers as $answer) {
                    ...
                    } elseif (LESSON_NEXTPAGE == $answer->jumpto) {
                        $jumptitle = get_string('nextpage', 'lesson');
                    } elseif (LESSON_EOL == $answer->jumpto) {
                ...
                if (LESSON_ENDOFBRANCH != $page->qtype) {
                    ...
                    echo "\" onclick=\"document.lessonpages.pageid.value=$page->id;" . 'document.lessonpages.submit();">';
                }
            ...
            if (isteacheredit($course->id)) {
                echo "<tr><td align=\"right\"><small><a href=\"import.php?id=$cm->id&pageid=$page->id\">" . get_string('importquestions', 'lesson') . '</a> | ' . "<a href=\"lesson.php?id=$cm->id&action=addbranchtable&pageid=$page->id\">" . get_string('addabranchtable', 'lesson') . '</a> | ';
                ...
                if ((LESSON_ENDOFBRANCH != $page->qtype) and (LESSON_ENDOFBRANCH != $nextqtype)) {
                    echo "<a href=\"lesson.php?id=$cm->id&action=addendofbranch&pageid=$page->id\">" . get_string('addanendofbranch', 'lesson') . '</a> | ';
                }

                echo "<a href=\"lesson.php?id=$cm->id&action=addpage&pageid=$page->id\">" . get_string('addaquestionpage', 'lesson') . ' ' . get_string('here', 'lesson') . "</a></small></td></tr>\n";
            }
        ...
        print_heading(
            "<a href=\"view.php?id=$cm->id&action=navigation\">" . get_string(
                'checknavigation',
                'lesson'
            ) . "</a>\n"
        );
