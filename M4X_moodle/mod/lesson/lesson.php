...
if ('addbranchtable' == $action) {
            ...
            <table cellpadding=5 border=1>
                ...
                <tr valign="top">
                    <td><b><?php print_string('pagetitle', 'lesson'); ?>:</b><br>
                        <input type="text" name="title" size="80" maxsize="255" value=""></td>
                </tr>
                <?php
                echo '<tr><td><b>';
    ...
    <?php
} /************** add end of branch ************************************/ elseif ('addendofbranch' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
                    ...
                    while ((LESSON_BRANCHTABLE != $btpage->qtype) and ($btpage->prevpageid > 0)) {
                    ...
                    if (LESSON_BRANCHTABLE == $btpage->qtype) {
                ...
                } /************** add page ************************************/ elseif ('addpage' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
    ...
    <?php
                } /******************* confirm delete ************************************/ elseif ('confirmdelete' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
                    ...
                    notice_yesno(
                        get_string('confirmdeletionofthispage', 'lesson'),
                        "lesson.php?action=delete&id=$cm->id&pageid=$pageid",
                        "view.php?id=$cm->id"
                    );
                } /****************** continue ************************************/ elseif ('continue' == $action) {
        ...
        case LESSON_BRANCHTABLE:
            ...
            if (0 == $newpageid) {
                $newpageid = $pageid;
            } elseif (LESSON_NEXTPAGE == $newpageid) {
            ...
            break;
                            ...
                            if (!$correctanswer and (0 == $newpageid)) {
                                ...
                                if ($nattempts >= $lesson->maxattempts) {
                                    if ($lesson->maxattempts > 1) { // don't bother with message if only one attempt
                                        echo '<p align="center">(' . get_string('maximumnumberofattempts', 'lesson') . ' ' . get_string('reached', 'lesson') . ' - ' . get_string('movingtonextpage', 'lesson') . ")</p>\n";
                                    }
                    ...
                    redirect("view.php?id=$cm->id", get_string('ok'));
                } /************** edit page ************************************/ elseif ('editpage' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
                    ...
                    while (true) {
                        if ($apageid) {
                            ...
                            if (LESSON_ENDOFBRANCH != $apage->qtype) {
            ...
            <table cellpadding=5 border=1>
                ...
                echo '<tr><td><b>';
                    ...
                    if ($answers = get_records('lesson_answers', 'pageid', $page->id, 'id')) {
                        foreach ($answers as $answer) {
                            $flags = (int)$answer->flags; // force into an integer
                            ...
                            case LESSON_MATCHING:
                                ...
                                if ($flags & LESSON_ANSWER_EDITOR) {
                                    echo ' [' . get_string('useeditor', 'lesson') . ': ' . "<input type=\"checkbox\" name=\"answereditor[$n]\" value=\"1\" 
                                checked=\"checked\">";
                                ...
                                if ($flags & LESSON_RESPONSE_EDITOR) {
                                    echo ' [' . get_string('useeditor', 'lesson') . ': ' . "<input type=\"checkbox\" name=\"responseeditor[$n]\" value=\"1\" 
                                checked=\"checked\">";
                    ...
                    if (LESSON_ENDOFBRANCH != $page->qtype) {
                        for ($i = $n; $i < $lesson->maxanswers; $i++) {
                            ...
                            case LESSON_MATCHING:
                                ...
                                echo ' [' . get_string('useeditor', 'lesson') . ': ' . "<input type=\"checkbox\" name=\"answereditor[$i]\" value=\"1\">";
                                helpbutton('useeditor', get_string('useeditor', 'lesson'), 'lesson');
                                echo "]<br>\n";
                                print_textarea(false, 10, 70, 630, 300, "answer[$i]");
                                echo "</td></tr>\n";
                                echo '<tr><td><b>' . get_string('response', 'lesson') . " $iplus1:</b>\n";
                                echo ' [' . get_string('useeditor', 'lesson') . ': ' . "<input type=\"checkbox\" name=\"responseeditor[$i]\" value=\"1\">";
                                helpbutton('useeditor', get_string('useeditor', 'lesson'), 'lesson');
                                echo "]<br>\n";
    ...
    <?php
                } /****************** insert page ************************************/ elseif ('insertpage' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
            ...
            $newpage->timecreated = $timenow;
                    ...
                    echo "<tr><td><a href=\"lesson.php?id=$cm->id&action=moveit&pageid=$pageid&after=0\"><small>" . get_string('movepagehere', 'lesson') . "</small></a></td></tr>\n";
                    ...
                    while (true) {
                        if ($page->id != $pageid) {
                            ...
                            echo "<tr><td><a href=\"lesson.php?id=$cm->id&action=moveit&pageid=$pageid&after={$page->id}\"><small>" . get_string('movepagehere', 'lesson') . "</small></a></td></tr>\n";
                        }
                    ...
                    redirect("view.php?id=$cm->id", get_string('ok'));
                } /****************** update page ************************************/ elseif ('updatepage' == $action) {
                    if (!isteacher($course->id)) {
                        error('Only teachers can look at this page');
                    }
            ...
            if (!isset($form->answereditor[$i])) { // clean up check box
                $form->answereditor[$i] = 0;
                    ...
                    $newanswer->lessonid = $lesson->id;
                                    ...
                                    $newanswer->flags = $form->answereditor[$i] * LESSON_ANSWER_EDITOR + $form->responseeditor[$i] * LESSON_RESPONSE_EDITOR;
