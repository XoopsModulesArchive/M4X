...
// $Id: view.php,v 1.15.2.9 2004/09/09 15:57:17 moodler Exp $
...
$moodle_disable_camel_case = (1 == $wiki->disablecamelcase);
...
if (($wiki_entry = wiki_get_default_entry($wiki, $course, $userid, $groupid))) {
    ...
    $wikipage = (false === $wikipage) ? stripslashes(EWIKI_PAGE_INDEX) : stripslashes($wikipage);
    ...
    if (isset($userid)) {
        $ewbase .= '&userid=' . $userid;
    }

    if (isset($groupid)) {
        $ewbase .= '&groupid=' . $groupid;
    }
    ...
    require_once $CFG->dirroot . '/mod/wiki/ewikimoodlelib.php';

    # Plugins

    //require_once $CFG->dirroot."/mod/wiki/ewiki/plugins/email_protect.php";

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/patchsaving.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/notify.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/feature/imgresize_gd.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/moodle_highlight.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/f_fixhtml.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/sitemap.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/moodle_wikidump.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/aview/backlinks.php';

    #require_once $CFG->dirroot."/mod/wiki/ewiki/plugins/markup/css.php";

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/markup/footnotes.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/diff.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/page/pageindex.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/page/orphanedpages.php';

    require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/wantedpages.php';
    ...
    if ($wiki->ewikiacceptbinary) {
        ...
        require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/lib/mime_magic.php';

        require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/aview/downloads.php';

        require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/downloads.php';

        #require_once $CFG->dirroot."/mod/wiki/ewiki/plugins/db/binary_store.php";

        require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/moodle_binary_store.php';
    } else {
    ...
    require_once $CFG->dirroot . '/mod/wiki/ewiki/ewiki.php';

    # Language-stuff: eWiki gets language from Browser. Lets correct it. Empty arrayelements do no harm

    $ewiki_t['languages'] = [current_language(), $course->lang, $CFG->lang, 'en', 'c'];
    ...
    if (0 == $wiki->htmlmode) {
        # No HTML
        $ewiki_config['htmlentities'] = []; // HTML is managed by moodle
    ...
    if (1 == $wiki->htmlmode) {
        ...
        require_once $CFG->dirroot . '/mod/wiki/ewiki/plugins/moodle/moodle_rescue_html.php';
    ...
    if (2 == $wiki->htmlmode) {
        ...
        $ewiki_config['htmlentities'] = []; // HTML is allowed
...
if ($wiki_list = wiki_get_other_wikis($wiki, $USER, $course, $wiki_entry->id)) {
    ...
    echo '<td class="sideblockheading" bgcolor="' . $THEME->cellheading . '">&nbsp;' . $WIKI_TYPES[$wiki->wtype] . ' ' . get_string('modulename', 'wiki') . ' ' . get_string('for', 'wiki') . ' ' . wiki_get_owner($wiki_entry) . ':</td>';

    echo '<td class="sideblockheading" bgcolor="' . $THEME->cellheading . '" align="right">' . get_string('otherwikis', 'wiki') . ':&nbsp;&nbsp;';
...
if ($wiki_entry) {
    $specialpages = ['WikiExport', 'SiteMap', 'SearchPages', 'PageIndex', 'NewestPages', 'MostVisitedPages', 'MostOftenChangedPages', 'UpdatedPages', 'FileDownload', 'FileUpload', 'OrphanedPages', 'WantedPages'];
    ...
    if ($canedit) {
        ...
        wiki_print_administration_actions($wiki, $cm->id, $userid, $groupid, $ewiki_title, 2 != $wiki->htmlmode, $course);
...
if (!empty($canedit)) {   /// Print tabs with commands for this page
    ...
    foreach ($tabs as $tab) {
        ...
        if ($ewiki_action != (string)$tab && !in_array($wikipage, $specialpages, true)) {
...
print_simple_box_start('right', '100%', (string)$THEME->cellcontent, '20');
