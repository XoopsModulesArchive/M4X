...
// $Id: admin.php,v 1.8.2.1 2004/10/01 02:18:33 moodler Exp $
...
/// Correct Action ?
if (!in_array($action, ['setpageflags', 'removepages', 'strippages', 'checklinks', 'revertpages'], true)) {
    error("Unknown action '$action'", 'wiki');
}

/// May the User administrate it ?
if (false === ($wiki_entry = wiki_get_entry($wiki, $course, $userid, $groupid)) || false === wiki_can_edit_entry($wiki_entry, $wiki, $USER, $course)) {
    error(get_string('notadministratewiki', 'wiki'));
}

# Check for dangerous events (hacking) !
if (in_array($action, ['removepages', 'strippages', 'revertpages'], true)) {
    if (!('student' == $wiki->wtype || isteacher($course->id))) {
...
if ($form = data_submitted()) {
    switch ($action) {
        case 'revertpages':
            if (!$form->deleteversions || $form->deleteversions < 0 || $form->deleteversions > 1000) {
                $focus = 'form.deleteversions';

                $err->deleteversions = get_string('deleteversionserror', 'wiki');
            }
            if (!$form->changesfield || $form->changesfield < 0 || $form->changesfield > 100000) {
...
print_header_simple(
    (string)$wiki_entry->pagename,
    '',
    "<A HREF=\"index.php?id=$course->id\">$strwikis</A> -> <A HREF=\"view.php?id=$id\">$wiki->name</a> ->" . get_string('administration', 'wiki'),
    $focus,
    '',
    true,
...
if ($form = data_submitted()) {
    ...
    switch ($action) {
        case 'removepages':
            $addloginfo = @implode(', ', $form->pagestodelete);
            break;
        case 'strippages':
            $addloginfo = @implode(', ', $form->pagestostrip);
            break;
        case 'checklinks':
    ...
    switch ($action) {
        case 'removepages':
            if ($form->proceed) {
                if (!$confirm && $form->pagestodelete) {
                    notice_yesno(
                        get_string('removepagecheck', 'wiki') . '<br>' . implode(', ', $form->pagestodelete),
                        $link . '&confirm=' . urlencode(implode(' ', $form->pagestodelete)),
                        $link
                    );
            ...
            if ($form->proceed) {
                if (!$confirm && $form->pagestostrip) {
                    ...
                    foreach ($strippages as $cnfid => $cnfver) {
                        $confirm .= "&confirm[$cnfid]=" . urlencode(implode(' ', $cnfver));
                    }

                    if (0 == count($err)) {
                        notice_yesno(
                            get_string('strippagecheck', 'wiki') . '<br>' . implode(', ', $form->pagestostrip),
                            $link . $confirm,
                            $link
                        );
        ...
        case 'checklinks':
            if ($form->proceed) {
                if (!$confirm && $form->pagetocheck) {
                    ...
                    notice_yesno(
                        get_string('checklinkscheck', 'wiki') . $form->pagetocheck,
                        $link . $confirm,
                        $link
                    );
        ...
        case 'revertpages':
            if (!$err) {
                if (!$confirm) {
                    $confirm = '&confirm[changesfield]=' . urlencode($form->changesfield) . '&confirm[authorfieldpattern]=' . urlencode($form->authorfieldpattern) . '&confirm[howtooperate]=' . urlencode($form->howtooperate) . '&confirm[deleteversions]=' . urlencode($form->deleteversions);

                    $revertedpages = wiki_admin_revert('', $form->authorfieldpattern, $form->changesfield, $form->howtooperate, $form->deleteversions);

                    if ($revertedpages) {
                        notice_yesno(
                            get_string('revertpagescheck', 'wiki') . '<br>' . $revertedpages,
                            $link . $confirm,
                            $link
                        );
            ...
            break;
        default:
            error("No such Wiki-Admin action: $action");
            break;
    }
}
...
if ($confirm && !$err) {
    switch ($action) {
        case 'removepages':
            $ret = wiki_admin_remove(preg_split(' ', $confirm), $course, $wiki, $userid, $groupid);
            if (!$ret) {
        ...
        case 'strippages':
            ...
            foreach ($confirm as $pageid => $versions) {
                $strippages[$pageid] = preg_split(' ', $versions);
            }
        ...
        case 'setpageflags':
            ...
            break;
        default:
            error("No such action '$action' with confirmation");
    }
}
...
if ($wiki_list = wiki_get_other_wikis($wiki, $USER, $course, $wiki_entry->id)) {
    ...
    echo '<td class="sideblockheading" bgcolor="' . $THEME->cellheading . '">&nbsp;' . $WIKI_TYPES[$wiki->wtype] . ' ' . get_string('modulename', 'wiki') . ' for ' . wiki_get_owner($wiki_entry) . ':</td>';
...
if ($wiki_entry) {
    ...
    wiki_print_administration_actions($wiki, $cm->id, $userid, $groupid, $wikipage, 2 != $wiki->htmlmode, $course);
...
// The wiki Contents
print_simple_box_start('center', '100%', (string)$THEME->cellcontent, '20');
// Do the Action
