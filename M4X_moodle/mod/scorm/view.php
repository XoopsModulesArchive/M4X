...
// $Id: view.php,v 1.9.2.1 2004/08/25 09:21:43 moodler Exp $
...
add_to_log($course->id, 'scorm', 'pre-view', "view.php?id=$cm->id", (string)$scorm->id);
//
...
if (!$cm->visible and !isteacher($course->id)) {
    ...
    print_header(
        $pagetitle,
        (string)$course->fullname,
        "$navigation <a target=\"{$CFG->framename}\" href=\"$ME?id=$cm->id\" title=\"$scorm->summary\">$scorm->name</a>",
        '',
        '',
        true,
        update_module_button($cm->id, $course->id, $strscorm),
    ...
    if ($orgs = get_records_select_menu('scorm_scoes', "scorm='$scorm->id' AND organization='' AND launch=''", 'id', 'id,title')) {
        if (count($orgs) > 1) {
            ...
            choose_from_menu($orgs, 'organization', (string)$organization, '', 'submit()');
        ...
        foreach ($scoes as $sco) {
            if ($parents[$level] != $sco->parent) {
                if ($level > 0 && $parents[$level - 1] == $sco->parent) {
                    ...
                    if ((0 == $i) && ($sco->parent != $orgidentifier)) {
            ...
            if ((false !== $nextsco) && ($sco->parent != $nextsco->parent) && ((0 == $level) || (($level > 0) && ($nextsco->parent == $sco->identifier)))) {
                $sublist++;
            ...
            if ($sco->launch) {
                ...
                if ($sco_user = get_record('scorm_sco_users', 'scoid', $sco->id, 'userid', $USER->id)) {
                    ...
                    echo '      <img src="pix/'
                         . scorm_remove_spaces($sco_user->cmi_core_lesson_status)
                         . '.gif" alt="'
                         . get_string(scorm_remove_spaces($sco_user->cmi_core_lesson_status), 'scorm')
                         . '" title="'
                         . get_string(scorm_remove_spaces($sco_user->cmi_core_lesson_status), 'scorm')
                         . "\">\n";

                    if (('not attempted' == $sco_user->cmi_core_lesson_status) || ('incomplete' == $sco_user->cmi_core_lesson_status)) {
                        $incomplete = true;
                    }
