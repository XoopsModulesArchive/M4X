...
// $Id: report.php,v 1.5.2.1 2004/11/10 14:18:49 moodler Exp $
...
add_to_log($course->id, 'scorm', 'report', "report.php?id=$cm->id", (string)$scorm->id);
    ...
    if ($sco_users = get_records_select('scorm_sco_users', "scormid='$scorm->id' GROUP BY userid")) {
        ...
        foreach ($sco_users as $sco_user) {
            ...
            if (is_array($user_data)) {
                ...
                $row[] = "<a href=\"$CFG->wwwroot/user/view.php?id=$data->userid&course=$course->id\">" . "$data->firstname $data->lastname</a>";
                ...
                foreach ($user_data as $data) {
                    ...
                    $row[] = '<img src="pix/' . scorm_remove_spaces($data->cmi_core_lesson_status) . '.gif" 
    						   ...
    						   title="' . get_string(scorm_remove_spaces($data->cmi_core_lesson_status), 'scorm') . '">&nbsp;' . $data->cmi_core_total_time . $scoreview;
                }
            }
