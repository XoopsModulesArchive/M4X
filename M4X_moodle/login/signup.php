...
// $Id: signup.php,v 1.38.2.1 2004/09/07 02:50:03 moodler Exp $

require_once '../config.php';
require_once "../auth/$CFG->auth/lib.php";

if ('email' != $CFG->auth and (empty($CFG->auth_user_create) or !(function_exists('auth_user_create')))) {
...
function validate_form($user, $err)
{
    ...
    if (isset($CFG->auth_user_create) and 1 == $CFG->auth_user_create and function_exists('auth_user_exists')) {
        if (auth_user_exists($user->username)) {
            $err->username = get_string('usernameexists');
        }
    }
    ...
    } elseif (record_exists('user', 'email', $user->email)) {
