...
// $Id: index.php,v 1.115.2.2 2004/09/07 14:14:30 moodler Exp $
...
if ($editing) {
    if (isset($_GET['blockaction'])) {
        if (isset($_GET['blockid'])) {
            block_action($site, $leftblocks, $rightblocks, mb_strtolower($_GET['blockaction']), (int)$_GET['blockid']);
        }
    }
    ...
    if ($editing && $recblocks) {
        foreach ($recblocks as $recblock) {
            ...
            if (!in_array($recblock->id, $allblocks, true) && !in_array(-($recblock->id), $allblocks, true)) {
                ...
                if ($formats['site'] ?? !empty($formats['all'])) {
...
if (!isset($SESSION) or !isset($SESSION->blockcache) or !isset($SESSION->blockcache->width->{$site->id}) or $editing) {
    ...
    <tr>
        ...
        if ($site->numsections > 0) {
            ...
            if ($editing) {
                ...
                echo "<a title=\"$streditsummary\" " . " href=\"course/editsection.php?id=$section->id\"><img src=\"$CFG->pixpath/t/edit.gif\" " . " height=11 width=11 border=0 alt=\"$streditsummary\"></a><br>";
            }
        ...
        switch ($CFG->frontpage) {     /// Display the main part of the front page.
            case FRONTPAGENEWS:
                ...
                if (isset($USER->id)) {
                    $SESSION->fromdiscussion = (string)$CFG->wwwroot;
