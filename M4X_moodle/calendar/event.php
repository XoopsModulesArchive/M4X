...
// $Id: event.php,v 1.22.4.1 2004/08/25 05:03:45 moodler Exp $
...
$_REQUEST['id'] = (int)$_REQUEST['id']; // Always a good idea, against SQL injections
...
if (!empty($_GET['course'])) {
    if (is_numeric($_GET['course']) && $_GET['course'] > 0 && record_exists('course', 'id', $_GET['course'])) {
        if (1 == $_GET['course']) {
            ...
            $SESSION->cal_courses_shown = (int)$_GET['course'];
...
switch ($_REQUEST['action']) {
    ...
    case 'new':
        ...
        $form = data_submitted();
        if (!empty($form) && 'defined' == $form->type) {
            $form->name = strip_tags($form->name);  // Strip all tags
...
if ($SESSION->cal_course_referer > 1
    && false !== ($shortname = get_field('course', 'shortname', 'id', $SESSION->cal_course_referer))) {
...
print_header(
    get_string('calendar', 'calendar') . ': ' . $title,
    $site->fullname,
    $nav . ' -> ' . $title,
    $focus,
    '',
...
switch ($_REQUEST['action']) {
    case 'delete':
        if (!empty($_REQUEST['confirm']) && 1 == $_REQUEST['confirm']) {
            // Kill it and redirect to day view

            if (false !== ($event = get_record('event', 'id', $_REQUEST['id']))) {
            ...
            print_side_block_start(get_string('deleteevent', 'calendar') . ': ' . $event->name, ['class' => 'mycalendar']);

            require __DIR__ . '/event_delete.html';
    ...
    case 'edit':
        ...
        print_side_block_start(get_string('editevent', 'calendar'), ['class' => 'mycalendar']);
        require __DIR__ . '/event_edit.html';
        print_side_block_end();
        if ($usehtmleditor) {
    ...
    case 'new':
        ...
        } elseif ($_GET['cal_y'] && $_GET['cal_m'] && checkdate($_GET['cal_m'], 1, $_GET['cal_y'])) {
        ...
        print_side_block_start(get_string('newevent', 'calendar') . $header, ['class' => 'mycalendar']);
        if ('select' == $_REQUEST['type']) {
            ...
            require __DIR__ . '/event_select.html';
        } else {
            require __DIR__ . '/event_new.html';
...
[$prevmon, $prevyr] = calendar_sub_month($mon, $yr);
[$nextmon, $nextyr] = calendar_add_month($mon, $yr);

echo calendar_filter_controls('event', 'action=' . $_REQUEST['action'] . '&amp;type=' . $_REQUEST['type'] . '&amp;id=' . $_REQUEST['id']);
echo '<p>';
echo calendar_top_controls('display', ['m' => $prevmon, 'y' => $prevyr]);
echo calendar_get_mini($courses, $groups, $users, $prevmon, $prevyr);
echo '</p><p>';
echo calendar_top_controls('display', ['m' => $mon, 'y' => $yr]);
echo calendar_get_mini($courses, $groups, $users, $mon, $yr);
echo '</p><p>';
echo calendar_top_controls('display', ['m' => $nextmon, 'y' => $nextyr]);
...
function validate_form($form, &$err)
{
    ...
    if (2 == $form->duration and !checkdate($form->endmon, $form->endday, $form->endyr)) {
        $err['timeduration'] = get_string('errorinvaliddate', 'calendar');
    }

    if (2 == $form->duration and !($form->minutes > 0 and $form->minutes < 1000)) {
    ...
    if (!empty($form->courseid)) {
        ...
        } elseif ($form->timestart < $course->startdate) {
...
function calendar_add_event_allowed($courseid, $groupid, $userid)
{
    ...
    if (isadmin()) {
        return true;
    } elseif (0 == $courseid && 0 == $groupid && $userid == $USER->id) {
        return true;
    } elseif (0 != $courseid && isteacheredit($courseid)) {
        return true;
    }
...
function calendar_get_allowed_types($allowed)
{
    ...
    if (!empty($SESSION->cal_course_referer) && $SESSION->cal_course_referer > 1 && isteacheredit($SESSION->cal_course_referer, $USER->id)) {
        $allowed->courses = [$SESSION->cal_course_referer => 1];
