...
// $Id: view.php,v 1.40.2.4 2004/11/05 06:28:16 moodler Exp $
...
print_header(
    "$site->shortname: $strcalendar: $pagetitle",
    $strcalendar,
    $nav,
    '',
    '',
...
function calendar_show_day($d, $m, $y, $courses, $groups, $users)
{
    ...
    if (!checkdate($m, $d, $y)) {
        ...
        [$d, $m, $y] = [(int)$now['mday'], (int)$now['mon'], (int)$now['year']];
    }
    ...
    print_side_block_start($text, ['class' => 'mycalendar']);

    echo '<p>' . calendar_top_controls('day', ['d' => $d, 'm' => $m, 'y' => $y]) . '</p>';
...
function calendar_show_month_detailed($m, $y, $courses, $groups, $users)
{
    ...
    if (!empty($m) && !empty($y)) {
        ...
        if ($m == $thisdate['mon'] && $y == $thisdate['year']) {
            ...
            if (!checkdate($m, 1, $y)) {
                $date = ['mday' => 1, 'mon' => $thisdate['mon'], 'year' => $thisdate['year']];

                $display->thismonth = true;
            } else {
                $date = ['mday' => 1, 'mon' => $m, 'year' => $y];
    ...
    [$d, $m, $y] = [$date['mday'], $date['mon'], $date['year']]; // This is what we want to display
    ...
    echo calendar_top_controls('month', ['m' => $m, 'y' => $y]);
    ...
    for ($day = 1; $day <= $display->maxdays; ++$day, ++$dayweek) {
        ...
        if (isset($eventsbyday[$day])) {
            ...
            } elseif (isset($typesbyday[$day]['startcourse'])) {
                $class .= ' cal_event_course';
            } elseif (isset($typesbyday[$day]['startgroup'])) {
                $class .= ' cal_event_group';
            } elseif (isset($typesbyday[$day]['startuser'])) {
                $class .= ' cal_event_user';
            }

            if (1 == count($eventsbyday[$day])) {
        ...
        } elseif (isset($typesbyday[$day]['durationcourse'])) {
            $class .= ' cal_duration_course';
        } elseif (isset($typesbyday[$day]['durationgroup'])) {
            $class .= ' cal_duration_group';
        } elseif (isset($typesbyday[$day]['durationuser'])) {
...
function calendar_course_filter_selector($getvars = '')
{
    ...
    $form = popup_form(
        CALENDAR_URL . 'set.php?var=setcourse&amp;' . $getvars . '&amp;id=',
        $courseoptions,
        'cal_course_flt',
        $selected,
        '',
