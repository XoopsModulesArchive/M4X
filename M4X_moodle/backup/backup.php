...
//$Id: backup.php,v 1.24.8.1 2004/09/08 22:44:42 stronk7 Exp $
...
if (!$id or $cancel) {
    print_header(
        "$site->shortname: $strcoursebackup",
        $site->fullname,
...
if (isadmin()) {
    ...
    print_header(
        "$course->shortname: $strcoursebackup",
        $course->fullname,
...
if (!$launch) {
    require_once 'backup_form.html';
} elseif ('check' == $launch) {
    require_once 'backup_check.html';
} elseif ('execute' == $launch) {
    require_once 'backup_execute.html';
}
print_simple_box_end();
