...
// $Id: setup.php,v 1.75.2.2 2004/10/15 08:46:13 moodler Exp $
//
// setup.php
//
...
if (!$dbconnected) {
    ...
    echo '<td style="color:#990000; text-align:center; font-size:large; border-width:1px; ' . '    border-color:#000000; border-style:solid; border-radius: 20px; border-collapse: collapse; ' . '    -moz-border-radius: 20px; padding: 15px">';
...
require_once "$CFG->dirxoops/class/smarty/Smarty.class.php";
//--------------------------------------------
$smarty = new Smarty();
...
if (!empty($CFG->opentogoogle)) {
    if (empty($_SESSION['USER'])) {
        if (!empty($_SERVER['HTTP_USER_AGENT'])) {
            if (false !== mb_strpos($_SERVER['HTTP_USER_AGENT'], 'Googlebot')) {
                $USER = guest_user();
            }

            if (false !== mb_strpos($_SERVER['HTTP_USER_AGENT'], 'google.com')) {
                $USER = guest_user();
            }
        }

        if (empty($_SESSION['USER']) and !empty(Request::getString('HTTP_REFERER', '', 'SERVER'))) {
            if (false !== mb_strpos(Request::getString('HTTP_REFERER', '', 'SERVER'), 'google')) {
                $USER = guest_user();
            }
        }
    }
