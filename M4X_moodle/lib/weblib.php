...
// $Id: weblib.php,v 1.282.2.16 2004/11/04 06:28:17 moodler Exp $
...
$ALLOWED_PROTOCOLS = ['http', 'https', 'ftp', 'news', 'mailto', 'rtsp', 'teamspeak', 'gopher', 'color'];
...
function s($var)
{
    ...
    return htmlspecialchars(stripslashes_safe($var), ENT_QUOTES | ENT_HTML5);
}
...
function p($var)
{
    ...
    echo htmlspecialchars(stripslashes_safe($var), ENT_QUOTES | ENT_HTML5);
}
...
function strip_querystring($url)
{
    ...
    if ($commapos = mb_strpos($url, '?')) {
        return mb_substr($url, 0, $commapos);
    }
...
function me()
{
    ...
    } elseif (!empty($_SERVER['URL'])) {     // May help IIS (not well tested)
        if (!empty($_SERVER['QUERY_STRING'])) {
...
function qualified_me()
{
    ...
    $protocol = (isset($_SERVER['HTTPS']) and 'on' == $_SERVER['HTTPS']) ? 'https://' : 'http://';
...
function match_referer($goodreferer = '')
{
    ...
    if ('nomatch' == $goodreferer) {   // Don't bother checking referer
        return true;
    }
...
function break_up_long_words($string, $maxsize = 20, $cutchar = ' ')
{
    ...
    if (in_array(current_language(), ['ja', 'zh_cn', 'zh_tw', 'zh_tw_utf8'], true)) {  // Multibyte languages
        return $string;
    }
...
if (!function_exists('str_ireplace')) {    /// Only exists in PHP 5
    function str_ireplace($find, $replace, $string)
    {
        ...
        foreach ($find as $fKey => $fItem) {
            $between = explode(mb_strtolower($fItem), mb_strtolower($string));

            $pos = 0;

            foreach ($between as $bKey => $bItem) {
                $between[$bKey] = mb_substr($string, $pos, mb_strlen($bItem));

                $pos += mb_strlen($bItem) + mb_strlen($fItem);
            }
...
function link_to_popup_window(
    ...
    $link = "<a target=\"$name\" title=\"$title\" href=\"$CFG->wwwroot$url\" " . "onClick=\"return openpopup('$url', '$name', '$options', $fullscreen);\">$linkname</a>\n";
...
function button_to_popup_window(
    $url,
    $name = 'popup',
    $linkname = 'click here',
    $height = 400,
    ...
    echo "<input type=\"button\" name=\"popupwindow\" title=\"$title\" value=\"$linkname ...\" " . "onClick=\"return openpopup('$url', '$name', '$options', $fullscreen);\">\n";
}
...
function close_window_button()
{
    ...
    echo '<a href="' . Request::getString('HTTP_REFERER', '', 'SERVER') . "\"><---</a>\n";
...
{
    if (mb_substr_count($string, '..') > $allowdots) {   // Sometimes we allow dots in references
        return true;
    }
...
function format_text_menu()
{
    ...
    return [
        FORMAT_MOODLE => get_string('formattext'),
    ...
    if (!empty($CFG->textfilters)) {
        ...
        foreach ($textfilters as $textfilter) {
            if (is_readable("$CFG->dirroot/$textfilter/filter.php")) {
                require_once "$CFG->dirroot/$textfilter/filter.php";
...
function cleanAttributes2($htmlTag)
{
    ...
    foreach ($attrArray as $arreach) {
        $attStr .= ' ' . mb_strtolower($arreach['name']) . '="' . $arreach['value'] . '" ';
    }
...
function replace_smilies(&$text)
{
    ...
    if (false === $runonce) {  /// After the first time this is not run again
...
function convert_urls_into_links(&$text)
{
    ...
    $text = eregi_replace(
        ...
        '\\1<a href="\\2://\\3\\4" target="newpage">\\2://\\3\\4</a>',
        $text
    );
    ...
    $text = eregi_replace(
        ...
        '\\1<a href="http://www.\\2\\3" target="newpage">www.\\2\\3</a>',
        $text
    );
}
...
function highlight(
    ...
    $haystack,
    $case = 0,
    $left_string = '<span class="highlight">',
    $right_string = '</span>'
) {
    ...
    for ($i = 0, $iMax = count($list_array); $i < $iMax; $i++) {
        if (1 == mb_strlen($list_array[$i])) {
            $list_array[$i] = '';
        }
    }
    ...
    if ('|' == $list_of_words_cp[mb_strlen($list_of_words_cp) - 1]) {
        $list_of_words_cp[mb_strlen($list_of_words_cp) - 1] = '';
    }

    $list_of_words_cp = '(' . trim($list_of_words_cp) . ')';

    if (!$case) {
        $haystack = eregi_replace((string)$list_of_words_cp, (string)$left_string . '\\1' . (string)$right_string, $haystack);
    } else {
        $haystack = preg_replace((string)$list_of_words_cp, (string)$left_string . '\\1' . (string)$right_string, $haystack);
    }
...
function highlightfast($needle, $haystack)
{
    ...
    $parts = explode(mb_strtolower($needle), mb_strtolower($haystack));

    $pos = 0;

    foreach ($parts as $key => $part) {
        $parts[$key] = mb_substr($haystack, $pos, mb_strlen($part));

        $pos += mb_strlen($part);

        $parts[$key] .= '<span class="highlight">' . mb_substr($haystack, $pos, mb_strlen($needle)) . '</span>';
...
function print_header(
        ...
        if (!empty($CFG->displayloginfailures)) {
            if (!empty($USER->username) and !isguest()) {
                if ($count = count_login_failures($CFG->displayloginfailures, $USER->username, $USER->lastlogin)) {
                    ...
                    if (isadmin()) {
                        $menu .= ' (<a href="' . $CFG->wwwroot . '/course/log.php' . '?chooselog=1&id=1&modid=site_errors">' . get_string('logs') . '</a>)';
                    }
    ...
    if ('rtl' == get_string('thisdirection')) {
        $direction = ' dir="rtl"';
    } else {
        $direction = ' dir="ltr"';
    }
    ...
    if ($usexml) {       // Added by Gustav Delius / Mad Alex for MathML output
        ...
        echo '//' . mb_strtoupper($currentlanguage) . "\" \"$CFG->xml_dtd\">\n";
...
function print_header_simple(
    ...
    print_header(
        "$course->shortname: $title",
        "$course->fullname $heading",
...
function print_footer($course = null, $usercourse = null)
{
    ...
    if ($course) {
        if ('home' == $course) {   // special case for site home page - please do not remove
...
function user_login_string($course, $user = null)
{
    ...
    if (isset($user->id) and $user->id) {
        ...
        if (isguest($user->id)) {
            $loggedinas = $realuserinfo . get_string('loggedinas', 'moodle', (string)$username) . " (<a target=\"{$CFG->framename}\" href=\"$wwwroot/login/index.php\">" . get_string('login') . '</a>)';
        } else {
            $loggedinas = $realuserinfo . get_string('loggedinas', 'moodle', (string)$username) . " (<a target=\"{$CFG->framename}\" href=\"$CFG->wwwroot/login/logout.php\">" . get_string('logout') . '</a>)';
        }
    } else {
        $loggedinas = get_string('loggedinnot', 'moodle') . " (<a target=\"{$CFG->framename}\" href=\"$wwwroot/login/index.php\">" . get_string('login') . '</a>)';
    }
...
function print_file_picture($path, $courseid = 0, $height = '', $width = '', $link = '')
{
    ...
    if ('http://' == mb_substr(mb_strtolower($path), 0, 7)) {
...
function print_user_picture($userid, $courseid, $picture, $large = false, $returnstring = false, $link = true)
{
    ...
    if ($picture) {  // Print custom user picture
        if ($CFG->slasharguments) {        // Use this method if possible for better caching
            $output .= "<img align=\"absmiddle\" src=\"$CFG->wwwroot/user/pix.php/$userid/$file.jpg\"" . " border=\"0\" width=\"$size\" height=\"$size\" alt=\"\">";
        } else {
            $output .= "<img align=\"absmiddle\" src=\"$CFG->wwwroot/user/pix.php?file=/$userid/$file.jpg\"" . " border=\"0\" width=\"$size\" height=\"$size\" alt=\"\">";
        }
    } else {         // Print default user pictures (use theme version if available)
        $output .= "<img align=\"absmiddle\" src=\"$CFG->pixpath/u/$file.png\"" . " border=\"0\" width=\"$size\" height=\"$size\" alt=\"\">";
    }
...
function print_user($user, $course)
{
    ...
    if (1 == $user->maildisplay or (2 == $user->maildisplay and $course->category and !isguest()) or $isteacher) {
...
function print_group_picture($group, $courseid, $large = false, $returnstring = false, $link = true)
{
    ...
    if ($group->picture) {  // Print custom group picture
        if ($CFG->slasharguments) {        // Use this method if possible for better caching
            $output .= "<img align=\"absmiddle\" src=\"$CFG->wwwroot/user/pixgroup.php/$group->id/$file.jpg\"" . " border=\"0\" width=\"$size\" height=\"$size\" alt=\"\" title=\"$group->name\">";
        } else {
            $output .= "<img align=\"absmiddle\" src=\"$CFG->wwwroot/user/pixgroup.php?file=/$group->id/$file.jpg\"" . " border=\"0\" width=\"$size\" height=\"$size\" alt=\"\" title=\"$group->name\">";
        }
    }
...
function print_png($url, $sizex, $sizey, $returnstring, $parameters = 'alt=""')
{
    ...
    if ($recentIE) {  // work around the HORRIBLE bug IE has with alpha transparencies
        $output .= "<img src=\"$CFG->pixpath/spacer.gif\" width=\"$sizex\" height=\"$sizey\"" . " border=\"0\" style=\"width: {$sizex}px; height: {$sizey}px; " . " filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='$url', sizingMethod='scale') " . " $parameters>";
    } else {
        $output .= "<img src=\"$url\" border=\"0\" width=\"$sizex\" height=\"$sizey\" " . " $parameters>";
    }
...
function print_table($table)
{
    ...
    print_simple_box_start('center', (string)$table->width, '#ffffff', 0);
...
function update_course_icon($courseid)
{
    ...
    if (isteacheredit($courseid)) {
        ...
        return "<form target=\"$CFG->framename\" method=\"get\" action=\"$CFG->wwwroot/course/view.php\">" . "<input type=\"hidden\" name=\"id\" value=\"$courseid\">" . "<input type=\"hidden\" name=\"edit\" value=\"$edit\">" . "<input type=\"submit\" value=\"$string\"></form>";
    }
}
...
function update_categories_button()
{
    ...
    if (isadmin()) {
        ...
        return "<form target=\"$CFG->framename\" method=\"get\" action=\"$CFG->wwwroot/course/index.php\">" . "<input type=\"hidden\" name=\"edit\" value=\"$edit\">" . "<input type=\"hidden\" name=\"sesskey\" value=\"$USER->sesskey\">" . "<input type=\"submit\" value=\"$string\"></form>";
    }
}
...
function update_groups_button($courseid)
{
    ...
    if (isteacheredit($courseid)) {
        ...
        return "<form target=\"$CFG->framename\" method=\"get\" action=\"$CFG->wwwroot/course/groups.php\">" . "<input type=\"hidden\" name=\"id\" value=\"$courseid\">" . "<input type=\"hidden\" name=\"edit\" value=\"$edit\">" . "<input type=\"submit\" value=\"$string\"></form>";
    }
}
...
function navmenu($course, $cm = null, $targetwindow = 'self')
{
    ...
    foreach ($modinfo as $mod) {
        ...
        if ($mod->section > 0 and $section != $mod->section) {
            ...
            if ($thissection->visible or !$course->hiddensections or $isteacher) {
                ...
                if ('weeks' == $course->format or empty($thissection->summary)) {
                    $menu[] = "-------------- $strsection $mod->section --------------";
                } else {
                    if (mb_strlen($thissection->summary) < 47) {
                        $menu[] = '-- ' . $thissection->summary;
                    } else {
                        $menu[] = '-- ' . mb_substr($thissection->summary, 0, 50) . '...';
                    }
                }
            }
        }
    ...
    if ($selectmod and $isteacher) {
        $logslink = "<td><a target=\"$CFG->framename\" href=" . "\"$CFG->wwwroot/course/log.php?chooselog=1&user=0&date=0&id=$course->id&modid=$selectmod->cm\">" . "<img border=\"0\" height=\"16\" width=\"16\" src=\"$CFG->pixpath/i/log.gif\"></a></td>";
    }

    if ($backmod) {
        $backmod = "<form action=\"$CFG->wwwroot/mod/$backmod->mod/view.php\" target=\"$CFG->framename\">" . "<input type=\"hidden\" name=\"id\" value=\"$backmod->cm\">" . '<input type="submit" value="&lt;"></form>';
    }

    if ($nextmod) {
        $nextmod = "<form action=\"$CFG->wwwroot/mod/$nextmod->mod/view.php\" target=\"$CFG->framename\">" . "<input type=\"hidden\" name=\"id\" value=\"$nextmod->cm\">" . '<input type="submit" value="&gt;"></form>';
    }

    return "<table><tr>$logslink<td>$backmod</td><td>" . popup_form(
        "$CFG->wwwroot/mod/",
        $menu,
        'navmenu',
        $selected,
...
function print_grade_menu($courseid, $name, $current, $includenograde = true)
{
    ...
    choose_from_menu($grades, (string)$name, (string)$current, '');
    ...
    link_to_popup_window(
        "/course/scales.php?id=$courseid&amp;list=true",
        'ratingscales',
        $linkobject,
        400,
        500,
...
function print_scale_menu($courseid, $name, $current)
{
    ...
    choose_from_menu(get_scales_menu($courseid), (string)$name, $current, '');
...
function print_scale_menu_helpbutton($courseid, $scale)
{
    ...
    link_to_popup_window(
        "/course/scales.php?id=$courseid&amp;list=true&amp;scale=$scale->id",
        'ratingscale',
        $linkobject,
        400,
        500,
...
function helpbutton($page, $title = '', $module = 'moodle', $image = true, $linktext = false, $text = '', $return = false)
{
    ...
    if ($image) {
        ...
        if ($linktext) {
            $linkobject = "<span style=\"cursor:help;\">$title<img align=\"absmiddle\" border=\"0\" " . " height=\"17\" width=\"22\" alt=\"\" src=\"$icon\"></span>";
        } else {
            $linkobject = '<img align="absmiddle" border="0" height="17" width="22" ' . " alt=\"$title\" style=\"cursor:help;\" src=\"$icon\">";
        }
    } else {
    ...
    if ($text) {
        $url = "/help.php?module=$module&amp;text=" . htmlentities(urlencode($text), ENT_QUOTES | ENT_HTML5);
    } else {
...
function notice($message, $link = '')
{
    ...
    print_simple_box($message, 'center', '50%', (string)$THEME->cellheading, '20', 'noticebox');
...
function notice_yesno($message, $linkyes, $linkno)
{
    ...
    print_simple_box_start('center', '60%', (string)$THEME->cellheading);
...
function obfuscate_mailto($email, $label = '', $dimmed = false)
{
    ...
    return sprintf(
        "<a href=\"%s:%s\" $dimmed title=\"$title\">%s</a>",
        obfuscate_text('mailto'),
...
function print_side_block_start($heading = '', $attributes = [])
{
    ...
    if (empty($attributes) || !is_array($attributes)) {
        ...
        $attributes['class'] = 'sideblock';
    } elseif (!mb_strpos($attributes['class'], 'sideblock')) {
        $attributes['class'] .= ' sideblock';
    }
...
function print_editor_config()
{
    ...
    $fontlist = isset($CFG->editorfontlist) ? explode(';', $CFG->editorfontlist) : [];
    ...
    foreach ($fontlist as $fontline) {
        if (!empty($fontline)) {
            [$fontkey, $fontvalue] = preg_split(':', $fontline);
