...
// $Id: lang.php,v 1.42.4.2 2004/10/03 09:11:11 stronk7 Exp $
...
print_header(
    "$site->shortname: $title",
    (string)$site->fullname,
    "<a href=\"index.php\">$stradministration</a> -> " . "<a href=\"configure.php\">$strconfiguration</a> -> $navigation",
    '',
    '',
    true,
    $button
...
if ('missing' == $mode and confirm_sesskey()) {
...
} elseif ('compare' == $mode and confirm_sesskey()) {
    ...
    print_heading((string)$currentfile, 'center', 4);
    ...
    if ($f = fopen("$langdir/$currentfile", 'r+b')) {
        $editable = true;
    ...
    foreach ($enstring as $key => $envalue) {
        $envalue = nl2br(htmlspecialchars($envalue, ENT_QUOTES | ENT_HTML5));
        ...
        if ($editable) {
            ...
            if (isset($string[$key])) {
                ...
                $valuelen = mb_strlen($envalue);
            }

            $cols = 50;

            if (mb_strstr($value, "\r") or mb_strstr($value, "\n") or $valuelen > $cols) {
...
function lang_save_file($path, $file, $strings)
{
    ...
    foreach ($strings as $key => $value) {
        [$id, $stringname] = explode('-', $key);

        if ('zh_hk' != $CFG->lang and 'zh_tw' != $CFG->lang) {  // Some MB languages include backslash bytes
            $value = str_replace('\\', '', $value);           // Delete all slashes
        }
