...
// $Id: filters.php,v 1.6.4.1 2004/10/03 00:28:22 stronk7 Exp $
...
print_header(
    "$site->shortname: $strmanagefilters",
    (string)$site->fullname,
    "<a href=\"index.php\">$stradministration</a> -> " . "<a href=\"configure.php\">$strconfiguration</a> -> $strmanagefilters"
);
...
foreach ($filterlocations as $filterlocation) {
    ...
    foreach ($plugins as $key => $plugin) {
        if (is_readable("$CFG->dirroot/$filterlocation/$plugin/filter.php")) {
            ...
            if (empty($name) or '[[filtername]]' == $name) {
                $name = $plugin;
            }
...
if (!empty($add) and !empty($uselect)) {
    ...
    if (!in_array($selectedfilter, $installedfilters, true)) {
        $installedfilters[] = $selectedfilter;

        set_config('textfilters', implode(',', $installedfilters));
    }
} elseif (!empty($remove) and !empty($iselect)) {
    ...
    set_config('textfilters', implode(',', $installedfilters));
} elseif ((!empty($up) or !empty($down)) and !empty($iselect)) {
    if (!empty($up)) {
