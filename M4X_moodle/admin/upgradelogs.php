...
//$Id: upgradelogs.php,v 1.3.8.1 2004/10/04 13:38:53 moodler Exp $
...
print_header(
    "$site->shortname: $stradministration: $strupgradinglogs",
...
if (empty($_GET['confirm'])) {
    notice_yesno(
        get_string('upgradelogsinfo', 'admin'),
...
if ($coursemodules = get_records_sql(
    "SELECT cm.*, m.name 
    ...
    if ($cmcount > 10) {
        print_simple_box(
            'This process may take a very long time ... please be patient and let it finish.',
            'center',
            '',
            '#ffcccc'
        );
    ...
    foreach ($coursemodules as $cm) {
        switch ($cm->name) {
            case 'forum':
                execute_sql(
                    "UPDATE {$CFG->prefix}log SET cmid = '$cm->id' 
                ...
                execute_sql(
                    "UPDATE {$CFG->prefix}log SET cmid = '$cm->id' 
                                 WHERE module = '$cm->name' AND url = 'view.php?f=$cm->instance'",
                    false
                );
                ...
                if ($discussions = get_records('forum_discussions', 'forum', $cm->instance)) {
                    foreach ($discussions as $discussion) {
                        execute_sql(
                            "UPDATE {$CFG->prefix}log SET cmid = '$cm->id' 
                                         WHERE module = '$cm->name' AND url $LIKE 'discuss.php?d=$discussion->id%'",
                            false
                        );
                    }
                }
            ...
            case 'assignment':
                ...
                execute_sql(
                    "UPDATE {$CFG->prefix}log SET cmid = '$cm->id' 
                                 WHERE module = '$cm->name' AND url = 'view.php?a=$cm->instance'",
                    false
                );
                execute_sql(
                    "UPDATE {$CFG->prefix}log SET cmid = '$cm->id' 
                                 WHERE module = '$cm->name' AND url = 'submissions.php?id=$cm->instance'",
                    false
                );
