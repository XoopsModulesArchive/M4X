...
// $Id: backup.php,v 1.7.8.1 2004/10/02 22:39:57 stronk7 Exp $
...
if ($config = data_submitted()) {
    ...
    if (!empty($backup_sche_destination) and ('/' == mb_substr($backup_sche_destination, -1) or '\\' == mb_substr($backup_sche_destination, -1))) {
        $error = true;

        $sche_destination_error = get_string('pathslasherror');

    //Now check that backup_sche_destination dir exists
    } elseif (!empty($backup_sche_destination) and !is_dir($backup_sche_destination)) {
        $error = true;
...
print_header(
    ...
    $site->fullname,
    "<a href=\"index.php\">$stradmin</a> -> " . "<a href=\"configure.php\">$strconfiguration</a> -> " . $strbackup
);
...
if (!function_exists('utf8_encode')) {
    print_simple_box('<font color="red">You need to add XML support to your PHP installation</font>', 'center', '70%', (string)$THEME->cellheading, '20', 'noticebox');
}
