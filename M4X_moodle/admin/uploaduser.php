...
// $Id: uploaduser.php,v 1.15.2.1 2004/10/03 16:03:44 stronk7 Exp $
...
'group5' => 1,
    ...
    $header = preg_split($csv_delimiter, fgets($fp, 1024));
    ...
    while (!feof($fp)) {
        ...
        $line = preg_split($csv_delimiter, fgets($fp, 1024));
        ...
        if ($record[$header[0]]) {
            ...
            foreach ($record as $name => $value) {
                ...
                if ($required[$name] and !$value) {
                    error(
                        get_string('missingfield', 'error', $name) . ' ' . get_string('erroronline', 'error', $linenum),
