...
// $Id: edit.php,v 1.62.4.4 2004/10/06 16:47:27 stronk7 Exp $
...
if ($usernew = data_submitted()) {
    ...
    if (find_form_errors($user, $usernew, $err)) {
        ...
        if ($usernew->url and !('http' == mb_substr($usernew->url, 0, 4))) {
...
if (($user->firstname and $user->lastname) or $newaccount) {
    ...
    if ($course->category) {
        print_header(
            "$course->shortname: $streditmyprofile",
            "$course->fullname: $streditmyprofile",
    ...
    print_header(
        "$course->shortname: $streditmyprofile",
        (string)$course->fullname,
        "<a href=\"$CFG->wwwroot/$CFG->admin/\">$stradministration</a> -> " . "<a href=\"$CFG->wwwroot/$CFG->admin/users.php\">$strusers</a> -> $straddnewuser",
        ''
    );
}

$teacher = mb_strtolower($course->teacher);
if (!isadmin()) {
...
print_heading(get_string('userprofilefor', '', (string)$userfullname));
...
function find_form_errors($user, $usernew, &$err)
{
    ...
    if (isadmin()) {
        ...
        } elseif (record_exists('user', 'username', $usernew->username) and 'changeme' == $user->username) {
        ...
        if (('admin' == $usernew->newpassword) or ($user->password == md5('admin') and empty($usernew->newpassword))) {
    ...
    } elseif ($otheruser = get_record('user', 'email', $usernew->email)) {
