...
// $Id: recent.php,v 1.15.4.2 2004/10/15 09:52:16 moodler Exp $
...
add_to_log($course->id, 'course', 'recent', "recent.php?id=$course->id", (string)$course->id);
...
if (!empty($_GET['chooserecent'])) {
    ...
    if ($course->category) {
        print_header(
            "$course->shortname: $strrecentactivity",
...
switch ($tmpmodid) {
    ...
    case 'activity/Chats':
        $filter = 'chat';
        break;
    case 'activity/Forums':
        $filter = 'forum';
        break;
    case 'activity/Quizzes':
        $filter = 'quiz';
        break;
    case 'activity/Workshops':
        $filter = 'workshop';
        break;
    default:
        $filter = '';
...
if (is_numeric($modid)) { // you chose a single activity
    $sections[0]->sequence = (string)$modid;
    ...
    $sections = get_records_sql(
        "SELECT cs.id, cs.section, cs.sequence, cs.summary, cs.visible
...
if (!empty($sections)) {
    ...
    foreach ($sections as $section) {
        ...
        foreach ($sectionmods as $sectionmod) {
            ...
            $coursemod = get_record_sql(
                "SELECT m.id, m.name, cm.groupmode, cm.visible
...
if (!empty($activities)) {
    ...
    foreach ($activities as $key => $activity) {
        ...
        if (('section' == $activity->type)
            && (($activity_count == ($key + 1))
                || ('section' == $activities[$key + 1]->type))) {
            continue;
        }

        if (('section' == $activity->type) && ('default' == $sortby)) {
            if ($inbox) {
            ...
            $inbox = true;
        } elseif ('activity' == $activity->type) {
            if ('default' == $sortby) {
                if ($teacher && 0 == $activity->visible) {
                    $linkformat = 'class="dimmed"';
                } else {
                    $linkformat = '';
                }

                $image = "<img src=\"$CFG->modpixpath/" . $activity->content->modname . '/icon.gif"' . 'height=16 width=16 alt="' . $activity->content->modfullname . '">';

                echo "<ul><h4>$image " . $activity->content->modfullname . " <a href=\"$CFG->wwwroot/mod/" . $activity->content->modname . '/view.php?' . 'id=' . $activity->content->modid . "\" $linkformat>" . $activity->name . '</a></h4></ul>';
            }
        } else {
            if (!$inbox) {
                print_simple_box_start('center', '90%');
