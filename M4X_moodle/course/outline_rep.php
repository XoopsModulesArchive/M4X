...
// $Id: outline_rep.php,v 1.2 2003/11/23 11:15:37 stronk7 Exp $
...
if ($course->category) {
    print_header(
        "$course->shortname: $stractivityreport",
...
print_heading((string)$course->fullname);
    ...
    if (isset($sections[$i])) {   // should always be true
        ...
        if ($section->sequence) {
            ...
            foreach ($sectionmods as $sectionmod) {
                ...
                if ($logs = get_records_select(
                    'log',
                    "module='$mod->modname'
                                           AND action LIKE 'view%' AND info='$mod->instance'",
                    'time ASC'
                )) {
                    $numviews = count($logs);
