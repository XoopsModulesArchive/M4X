...
// $Id: teacher.php,v 1.29.8.2 2004/10/09 19:01:02 stronk7 Exp $
...
if (!(isteacheredit($course->id) and iscreator()) and !(isteacheredit($course->id) and !empty($CFG->teacherassignteachers))) {
...
print_header(
    "$course->shortname: $strassignteachers",
    (string)$course->fullname,
    "<a href=\"index.php\">$strcourses</a> -> " . "<a href=\"view.php?id=$course->id\">$course->shortname</a> -> " . (string)$strassignteachers,
    ''
);
...
if ($form = data_submitted() and confirm_sesskey()) {
    ...
    foreach ($rank as $num => $vals) {
        if (!$teacher = get_record('user_teachers', 'course', (string)$course->id, 'userid', (string)$num)) {
...
if (empty($teachers)) {
    ...
    $table->head = ['', $strname, $strorder, $strrole, $stredit, '&nbsp'];

    $table->align = ['right', 'left', 'center', 'center', 'center', 'center'];
...
if (0 == $usercount) {
...
} elseif ($usercount > MAX_USERS_PER_PAGE) {
    ...
    $table->head = ['', get_string('name'), get_string('email'), ''];

    $table->align = ['right', 'left', 'center', 'center'];
    ...
    foreach ($users as $user) {
        ...
        $table->data[] = [$picture, fullname($user, true), $user->email, $addlink];
    }
