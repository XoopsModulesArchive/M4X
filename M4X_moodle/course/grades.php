...
// $Id: grades.php,v 1.21.2.1 2004/10/09 11:36:10 stronk7 Exp $
...
foreach ($students as $student) {
    $grades[$student->id] = [];    // Collect all grades in this array
    $gradeshtml[$student->id] = []; // Collect all grades html formatted in this array
    $totals[$student->id] = [];    // Collect all totals in this array
}
$columns = [];     // Accumulate column names in this array.
$columnhtml = [];  // Accumulate column html in this array.
    ...
    if (isset($sections[$i])) {   // should always be true
        ...
        if ($section->sequence) {
            ...
            foreach ($sectionmods as $sectionmod) {
                ...
                $instance = get_record((string)$mod->modname, 'id', (string)$mod->instance);
                ...
                if (file_exists($libfile)) {
                    ...
                    if (function_exists($gradefunction)) {   // Skip modules without grade function
                        if ($modgrades = $gradefunction($mod->instance)) {
                            ...
                            $image = "<A HREF=\"$CFG->wwwroot/mod/$mod->modname/view.php?id=$mod->id\"" . "   TITLE=\"$mod->modfullname\">" . "<IMG BORDER=0 VALIGN=absmiddle SRC=\"../mod/$mod->modname/icon.gif\" " . "HEIGHT=16 WIDTH=16 ALT=\"$mod->modfullname\"></A>";

                            if ($mod->visible) {
                                $columnhtml[] = "$image " . "<A HREF=\"$CFG->wwwroot/mod/$mod->modname/view.php?id=$mod->id\">" . (string)$instance->name . "</A>$maxgradehtml";
                            } else {
                                $columnhtml[] = "$image " . "<A CLASS=\"dimmed\" HREF=\"$CFG->wwwroot/mod/$mod->modname/view.php?id=$mod->id\">" . (string)$instance->name . "</A>$maxgradehtml";
                            }
                            ...
                            foreach ($students as $student) {
                                if (!empty($modgrades->grades[$student->id])) {
                                    ...
                                    if ($mod->visible) {
                                        ...
                                        $gradeshtml[$student->id][] = '<FONT class="dimmed_text">' . $modgrades->grades[$student->id] . '</FONT>';
                                    }
                                } else {
...
if ('xls' == $download and confirm_sesskey()) {
    ...
    exit;
} elseif ('txt' == $download and confirm_sesskey()) {
    ...
    $table->head = array_merge(['', get_string('firstname'), get_string('lastname')], $columnhtml, [get_string('total')]);
    $table->width = [35, ''];
    $table->align = ['LEFT', 'RIGHT', 'LEFT'];
    foreach ($columns as $column) {
        $table->width[] = '';
    ...
    foreach ($students as $key => $student) {
        ...
        $name = [(string)$picture, (string)$student->firstname, (string)$student->lastname];
