...
// $Id: category.php,v 1.39.4.3 2004/10/18 05:57:09 moodler Exp $
...
if ($creatorediting) {
    if ($adminediting) {
        print_header(
            "$site->shortname: $category->name",
            "$site->fullname: $strcourses",
            "<a href=\"../$CFG->admin/index.php\">$stradministration</a> -> " . "<a href=\"index.php\">$strcategories</a> -> $category->name",
            '',
            '',
            true,
            $navbaritem
...
echo '</td><td>';
popup_form('category.php?id=', $displaylist, 'switchcategory', (string)$category->id, '', '', '', false);
echo '</td></tr></table><br>';
        ...
        if ($swapcourse and $movecourse) {        // Renumber everything for robustness
            ...
            foreach ($courses as $course) {
                ...
                if ($course->id == $swapcourse->id) {
                    $course = $movecourse;
                } elseif ($course->id == $movecourse->id) {
                    $course = $swapcourse;
                }
...
} elseif ($numcourses <= COURSE_MAX_SUMMARIES_PER_PAGE and !$page and !$creatorediting) {
    ...
    foreach ($courses as $acourse) {
        ...
        if ($creatorediting) {
            if ($adminediting) {
                ...
                echo "<a title=\"$strsettings\" href=\"$CFG->wwwroot/course/edit.php?id=$acourse->id\"><img" . " src=\"$pixpath/t/edit.gif\" height=11 width=11 border=0></a> ";

                echo "<a title=\"$strassignteachers\" href=\"$CFG->wwwroot/course/teacher.php?id=$acourse->id\"><img" . " src=\"$pixpath/t/user.gif\" height=11 width=11 border=0></a> ";

                echo "<a title=\"$strdelete\" href=\"delete.php?id=$acourse->id\"><img" . " src=\"$pixpath/t/delete.gif\" height=11 width=11 border=0></a> ";

                if (!empty($acourse->visible)) {
                    echo "<a title=\"$strhide\" href=\"category.php?id=$category->id&hide=$acourse->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/hide.gif\" height=11 width=11 border=0></a> ";
                } else {
                    echo "<a title=\"$strshow\" href=\"category.php?id=$category->id&show=$acourse->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/show.gif\" height=11 width=11 border=0></a> ";
                }

                echo "<a title=\"$strbackup\" href=\"../backup/backup.php?id=$acourse->id\"><img" . " src=\"$pixpath/t/backup.gif\" height=11 width=11 border=0></a> ";

                echo "<a title=\"$strrestore\" href=\"../files/index.php?id=$acourse->id&wdir=/backupdata\"><img" . " src=\"$pixpath/t/restore.gif\" height=11 width=11 border=0></a> ";

                if ($up) {
                    echo "<a title=\"$strmoveup\" href=\"category.php?id=$category->id&moveup=$acourse->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/up.gif\" height=11 width=11 border=0></a> ";
                } else {
                    echo "<img src=\"$CFG->wwwroot/pix/spacer.gif\" height=11 width=11 border=0></a> ";
                }

                if ($down) {
                    echo "<a title=\"$strmovedown\" href=\"category.php?id=$category->id&movedown=$acourse->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/down.gif\" height=11 width=11 border=0></a> ";
                } else {
                    echo "<img src=\"$CFG->wwwroot/pix/spacer.gif\" height=11 width=11 border=0></a> ";
                }
            ...
            } elseif (isteacheredit($acourse->id)) {
                echo '<td>';
