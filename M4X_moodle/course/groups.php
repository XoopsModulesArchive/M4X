...
// $Id: groups.php,v 1.15.2.1 2004/10/09 12:01:34 stronk7 Exp $
...
if (!isteacheredit($course->id) and VISIBLEGROUPS != $course->groupmode) {
...
print_header(
    ...
    '',
    true,
    update_groups_button($course->id),
    $loggedinas
);
...
if (empty($USER->groupsediting)) {         /// Display an overview of all groups
    if (!$groups) {
        ...
        foreach ($groups as $group) {
            ...
            require __DIR__ . '/groups-summary.html';
        }
    }
...
if ($data = data_submitted() and confirm_sesskey()) {
    if (!empty($data->nonmembersadd)) {            /// Add people to a group
    ...
    } elseif (!empty($data->nonmembersrandom)) {  /// Add all non members to groups
        notify('Random adding of people into groups is not functional yet.');
    } elseif (!empty($data->nonmembersinfo)) {    /// Return info about the selected users
        notify('You must turn Javascript on');
    } elseif (!empty($data->groupsremove)) {      /// Remove a group, all members become nonmembers
    ...
    } elseif (!empty($data->groupsinfo)) {        /// Display full info for a group
        notify('You must turn Javascript on');
    } elseif (!empty($data->groupsadd)) {         /// Create a new group
    ...
    } elseif (!empty($data->membersremove)) {     /// Remove selected people from a particular group
        ...
        $selectedgroup = $data->groupid;
    } elseif (!empty($data->membersinfo)) {       /// Return info about the selected users
        notify('You must turn Javascript on');
    }
}
...
require __DIR__ . '/groups-edit.html';
