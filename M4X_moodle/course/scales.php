...
// $Id: scales.php,v 1.8.8.1 2004/10/09 18:30:11 stronk7 Exp $
...
if ('sendform' == $action and confirm_sesskey()) {
...
if ('details' == $action) {
    ...
    print_header(
        "$course->shortname: $strscales",
        (string)$course->fullname,
        "$course->shortname -> $strscales -> $scale->name",
        '',
        '',
        true,
        '&nbsp;',
...
if ('edit' == $action || 'new' == $action) {
    ...
    if (0 == $scale->courseid and !isadmin()) {
    ...
    print_header(
        "$course->shortname: $strscales",
        (string)$course->fullname,
        "<a href=\"view.php?id=$course->id\">$course->shortname</a>" . " -> <a href=\"scales.php?id=$course->id\">$strscales</a>" . ' -> ' . get_string('editinga', '', $strscale),
        $focus
    );
...
if ('delete' == $action and confirm_sesskey()) {
    //Check for teacher edit
...
if (('down' == $action || 'up' == $action) and confirm_sesskey()) {
    //Check for teacher edit
...
if (isset($_GET['list'])) {       /// Just list the scales (in a helpwindow)
    ...
    if (isset($_GET['scale'])) {
        if ($scale = get_record('scale', 'id', (string)$scale)) {
            $scalemenu = make_menu_from_list($scale->scale);
    ...
    if ($scales = get_records('scale', 'courseid', (string)$course->id, 'name ASC')) {
...
$scales = [];
$customscales = get_records('scale', 'courseid', (string)$course->id, 'name ASC');
...
if ($scales) {
    ...
    foreach ($scales as $scale) {
        ...
        $line[] = "<a target=\"scale\" title=\"$scale->name\" href=\"$CFG->wwwroot/course/scales.php?id=$course->id&scaleid=$scale->id&action=details\" "
                  . "onClick=\"return openpopup('/course/scales.php?id=$course->id\&scaleid=$scale->id&action=details', 'scale', 'menubar=0,location=0,scrollbars,resizable,width=600,height=450', 0);\">"
                  . $scale->name
                  . '</a><br><font size="-1">'
        ...
        if (empty($scales_uses) && ($incustom || isadmin())) {
            $buttons .= "<a title=\"$stredit\" href=\"$path/scales.php?id=$course->id&scaleid=$scale->id&action=edit\"><img" . " src=\"$pixpath/t/edit.gif\" hspace=2 height=11 width=11 border=0></a> ";

            if ($incustom && isadmin()) {
                $buttons .= "<a title=\"$strdown\" href=\"$path/scales.php?id=$course->id&scaleid=$scale->id&action=down&amp;sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/down.gif\" hspace=2 height=11 width=11 border=0></a> ";
            }

            if (!$incustom && isadmin()) {
                $buttons .= "<a title=\"$strup\" href=\"$path/scales.php?id=$course->id&scaleid=$scale->id&action=up&amp;sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/up.gif\" hspace=2 height=11 width=11 border=0></a> ";
            }

            $buttons .= "<a title=\"$strdelete\" href=\"$path/scales.php?id=$course->id&scaleid=$scale->id&action=delete&amp;sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/delete.gif\" hspace=2 height=11 width=11 border=0></a> ";
        }
