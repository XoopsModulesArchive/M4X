...
// $Id: index.php,v 1.29.4.2 2004/10/09 13:11:48 stronk7 Exp $
...
if ((isset($moveup) or isset($movedown)) and confirm_sesskey()) {
    ...
    if (isset($moveup)) {
        if ($movecategory = get_record('course_categories', 'id', $moveup)) {
            $categories = get_categories((string)$movecategory->parent);
    ...
    if ($swapcategory and $movecategory) {        // Renumber everything for robustness
        ...
        foreach ($categories as $category) {
            ...
            } elseif ($category->id == $movecategory->id) {
...
function print_category_edit($category, $displaylist, $parentslist, $depth = -1, $up = false, $down = false)
{
    ...
    if ($category) {
        ...
        echo "<a title=\"$str->delete\" href=\"index.php?delete=$category->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/delete.gif\" height=11 width=11 border=0></a> ";

        if (!empty($category->visible)) {
            echo "<a title=\"$str->hide\" href=\"index.php?hide=$category->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/hide.gif\" height=11 width=11 border=0></a> ";
        } else {
            echo "<a title=\"$str->show\" href=\"index.php?show=$category->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/show.gif\" height=11 width=11 border=0></a> ";
        }

        if ($up) {
            echo "<a title=\"$str->moveup\" href=\"index.php?moveup=$category->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/up.gif\" height=11 width=11 border=0></a> ";
        }

        if ($down) {
            echo "<a title=\"$str->movedown\" href=\"index.php?movedown=$category->id&sesskey=$USER->sesskey\"><img" . " src=\"$pixpath/t/down.gif\" height=11 width=11 border=0></a> ";
        }
        ...
        foreach ($parentslist as $key => $parents) {
            if (in_array($category->id, $parents, true)) {
                unset($tempdisplaylist[$key]);
            }
        }

        popup_form("index.php?move=$category->id&sesskey=$USER->sesskey&moveto=", $tempdisplaylist, "moveform$category->id", (string)$category->parent, '', '', '', false);
