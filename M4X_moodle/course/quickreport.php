...
// $Id: quickreport.php,v 1.2 2004/07/25 14:00:29 moodler Exp $
...
$quick_dataloc['assignment'] = ['assignment_submissions', 'assignment'];
$quick_dataloc['choice'] = ['choice_answers', 'choice'];
$quick_dataloc['dialogue'] = ['dialogue_entries', 'dialogue'];
$quick_dataloc['discussion'] = ['forum_posts', 'discussion'];
$quick_dataloc['journal'] = ['journal_entries', 'journal'];
$quick_dataloc['quiz'] = ['quiz_attempts', 'quiz'];
$quick_dataloc['survey'] = ['survey_analysis', 'survey'];
$quick_dataloc['workshop'] = ['workshop_submissions', 'workshopid'];
...
foreach ($modlist as $nowmod) {
    if (mb_strpos($usemods, $nowmod->name)) {
    ...
    if ($allrecs) {
        foreach ($allrecs as $rec) {
            ...
            if ('assignment' == $nowname) {
                ...
                if (0 != $rec->timemodified) {
                    $user_data[$nowtype][$np] .= ",$rec->userid,";
                }
            } else {
                $user_data[$nowtype][$np] .= ",$rec->userid,";
            }
...
if ($allposts) {
    foreach ($allposts as $rec) {
        ...
        $user_data[$forumval][$rec->forumid] .= ",$rec->userid,";
...
foreach ($allusers as $user) {
    ...
    foreach ($mod_info as $thismod) {
        if (mb_strlen($user_data[$thismod[module]][$thismod[instance]]) < 2) {
            $user_contribs[$uid][] = '-';
        ...
        if ($thismod[module] == $modname_value['forum']) {
            ...
            if (mb_strpos($user_data[$thismod[module]][$thismod[instance]], $sstring) > -1) {
                $user_contribs[$uid][] = 'X';
            } else {
                $user_contribs[$uid][] = '-';
            }
...
if ('xls' == $download) {
    ...
    foreach ($user_contribs as $userid => $thisuser) {
        ...
        if (mb_strlen($allusers[$userid]->description) > 10) {
            $myxls->write_string($row, $col, 'X', $formatc);
        } else {
            $myxls->write_string($row, $col, '-', $formatc);
        }
...
function quick_findusers($courseid)
{
    ...
    $allusers = get_records_sql(
        "SELECT a.id,a.lastname, a.firstname,a.picture,a.description,b.course
...
function quick_findrecs($tbl1, $tbl2, $pointer, $courseid)
{
    ...
    $allrecs = get_records_sql(
        "SELECT b.id,a.id, c.id,c.lastname, c.firstname,a.course,a.name ,b.userid,b.timemodified, b.$pointer as instance
    ...
    foreach ($allmods as $thisweek) {
        ...
        if ($thisweek->sequence) {
            ...
            foreach ($weeklist as $activity) {
                $all_list[] = ['mod' => $activity, 'sec' => $thisweek->section];
            }
